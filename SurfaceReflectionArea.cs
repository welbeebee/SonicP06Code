...
public class SurfaceReflectionArea : MonoBehaviour
{
	...
	private void Start()
	{
		...
		{
			for (int i = 0; i < reflectionArea.Materials.Length; i++)
			{
				reflectionArea.Materials[i].DisableKeyword("PLANARREF");
			}
		}
	}
	...
	private void Update()
	{
		...
		if (reflectionArea == null || reflectionArea.Renderer == null || !reflectionArea.Renderer.enabled || reflectionArea.Materials == null)
		{
			return;
		}
		for (int i = 0; i < reflectionArea.Materials.Length; i++)
		{
			...
			{
				reflectionArea.Materials[i].EnableKeyword("PLANARREF");
				reflectionArea.Materials[i].SetTexture("_ReflectionTex", m_ReflectionTexture);
			}
			else
			{
				reflectionArea.Materials[i].DisableKeyword("PLANARREF");
