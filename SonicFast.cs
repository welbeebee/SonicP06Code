...
public class SonicFast : PlayerBase
{
	...
	public override void Awake()
	{
		...
		JumpWalkSpeed = Sonic_Fast_Lua.c_slow_speed_max;
		JumpTopSpeed = Sonic_Fast_Lua.c_walk_speed_max;
	}
	...
	private void StateJump()
	{
		...
		JumpAnimation = ((!(AirMotionVelocity.y > -3f)) ? 2 : ((Time.time - JumpTime > Sonic_Fast_Lua.c_jump_time_min || JumpBounce) ? 1 : 0));
		if (!Singleton<RInput>.Instance.P.GetButton("Button A"))
		{
			ReleasedKey = true;
		}
		if (Singleton<GameManager>.Instance.GameState != GameManager.State.Paused && !ReleasedKey && Singleton<RInput>.Instance.P.GetButton("Button A") && Time.time - JumpTime < (JumpBounce ? 0.25f : ((!HalveSinkJump) ? 0.4f : 0.25f)))
		{
			AirMotionVelocity += Vector3.up * ((!HalveSinkJump) ? 12.25f : 11f) * Time.fixedDeltaTime * ((!JumpBounce) ? 2f : 4.75f);
		}
		if (JumpAttackSphere(base.transform.position, 0.5f, base.transform.forward * _Rigidbody.velocity.magnitude, 1))
		{
			...
			HalveSinkJump = false;
			JumpAnimation = 1;
			AirMotionVelocity.y = Sonic_Fast_Lua.c_jump_speed * 1.5f;
		}
	...
	private void StateLightDash()
	{
		...
		{
			CurSpeed = ((VerticalDir() > 0f) ? Speed : Sonic_Fast_Lua.c_run_speed_max);
			if (IsGrounded())
			{
				StateMachine.ChangeState(StateGround);
			}
	...
	private void StateSlide()
	{
		...
		GeneralMeshRotation = Quaternion.LookRotation(ForwardMeshRotation, UpMeshRotation);
		base.transform.rotation = Quaternion.FromToRotation(base.transform.up, RaycastHit.normal) * base.transform.rotation;
		SlideSpeed = Mathf.Clamp(SlideSpeed, TopSpeed, Sonic_Fast_Lua.c_lightdash_speed);
	...
	private void StateBoundAttack()
	{
		...
		else if (BoundState == 1)
		{
			...
			if (Time.time - BoundTime < 0.25f)
			{
				...
				if (CurSpeed > TopSpeed * 0.8f)
				{
					CurSpeed = TopSpeed * 0.8f;
				}
	...
	private void StateHurt()
	{
		...
		HurtStartSpeed = Mathf.MoveTowards(HurtStartSpeed, WalkSpeed, maxDelta);
	...
	private void StateWallSlam()
	{
		...
		CurSpeed = Mathf.Lerp(0f - WalkSpeed, 0f, num * 1.25f);
		_Rigidbody.velocity = base.transform.forward * CurSpeed;
		GeneralMeshRotation = Quaternion.LookRotation(ForwardMeshRotation, UpMeshRotation);
		base.transform.rotation = Quaternion.FromToRotation(base.transform.up, RaycastHit.normal) * base.transform.rotation;
	...
	private void StateAirSlam()
	{
		...
		CurSpeed = Mathf.Lerp(0f - WalkSpeed, 0f, t);
		Vector3 vector = new Vector3(AirMotionVelocity.x, 0f, AirMotionVelocity.z);
		if (_Rigidbody.velocity.magnitude != 0f)
		{
			vector = base.transform.forward * CurSpeed;
	...
	private void ControlSpeed()
	{
		...
		bool flag = CurSpeed > ((!IsSuper) ? Sonic_Fast_Lua.c_lightdash_mid_speed : Sonic_Fast_Lua.c_lightdash_mid_speed_super);
		if (VerticalDir() > 0f)
		{
			if (flag)
			{
		...
		else if (CurSpeed < TopSpeed)
		{
			CurSpeed += num;
		}
		else if (CurSpeed > TopSpeed)
		{
			CurSpeed -= num * ((CurSpeed > Sonic_Fast_Lua.c_run_speed_max) ? 7.5f : 1f);
		}
	}
	...
	public override void Update()
	{
		...
		UseSpeedBarrier = (PlayerState == State.Ground || PlayerState == State.Jump || PlayerState == State.Air || PlayerState == State.LightDash) && CurSpeed >= 83f;
		CanSink = !IsSuper;
		if (Singleton<GameManager>.Instance.GameState != GameManager.State.Paused && Singleton<GameManager>.Instance.GameState != GameManager.State.Result && StageManager.StageState != StageManager.State.Event && !IsDead && PlayerState != State.Talk)
		{
			if (PlayerState == State.Ground && CanJumpFromSink())
			{
				...
				if (CurSpeed > WalkSpeed && Singleton<RInput>.Instance.P.GetButtonDown("Button B") && !IsSinking)
				{
	...
	public override void UpdateAnimations()
	{
		...
		Steering = Mathf.MoveTowards(Steering, (!LockControls && UseSpeedBarrier && UseCharacterSway) ? HorizontalDir() : 0f, Time.deltaTime * 3.5f);
