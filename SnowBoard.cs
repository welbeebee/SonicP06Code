...
public class SnowBoard : PlayerBase
{
	...
	public override void Awake()
	{
		...
		if (stage == StageManager.Stage.wap)
		{
			...
			JumpWalkSpeed = Snow_Board_Wap_Lua.c_jump_walk;
			JumpTopSpeed = Snow_Board_Wap_Lua.c_jump_run;
		...
		else
		{
			...
			JumpWalkSpeed = Snow_Board_Lua.c_jump_walk;
			JumpTopSpeed = Snow_Board_Lua.c_jump_run;
	...
	private void StateJump()
	{
		...
		JumpMaxSpeed = Mathf.Lerp(JumpMaxSpeed, JumpTopSpeed, Time.fixedDeltaTime);
	...
	public override void Update()
	{
		...
		if (PlayerState == State.Ground)
		{
			if (Singleton<RInput>.Instance.P.GetButton("Button A"))
			{
				...
				BoardDecel = 3f;
				JumpStrenght = Mathf.Min(JumpStrenght + BaseJump, JumpTopSpeed);
			}
	...
	public override bool IsInvulnerable(int HurtType)
	{
		...
		if (PlayerState == State.Talk || PlayerState == State.WarpHole || PlayerState == State.Result || PlayerState == State.Cutscene || HasInvincibility || IsDead)
		{
			return true;
		}
		return result;
	...
	public override void UpdateAnimations()
	{
		...
		LookBackAnim = Mathf.Lerp(LookBackAnim, IsLookBack ? 1f : 0f, Time.deltaTime * 5f);
	...
	public void OnRampEnter(Vector3 _RampVelocity, float _RampTime, float _RampRate, float _RampBPower_Rate, bool _RampLockMove)
	{
		RampVelocity = _RampVelocity;
		RampTime = _RampTime;
		RampRate = _RampRate;
		RampBPower_Rate = _RampBPower_Rate;
		RampLockMove = _RampLockMove;
	}
