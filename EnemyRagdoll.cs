...
public class EnemyRagdoll : MonoBehaviour
{
	...
	internal LayerMask Break_Mask => LayerMask.GetMask("Default", "DefaultObject", "PlayerCollision", "PlayerCollisionObject", "BreakableObj");
	...
	private void Explode()
	{
		if ((ExplodeOnCollision || IsChainTarget) && !Exploded)
		{
			...
			Object.Instantiate(ExplosionFX, Center.position, Quaternion.identity);
			if ((bool)Chain)
			{
				Chain.Detonate();
			}
	...
	private void OnBreak(Vector3 position, HitInfo HitInfo)
	{
		Collider[] array = Physics.OverlapSphere(position, Radius, Break_Mask);
