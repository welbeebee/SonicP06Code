...
public class Eggtrain : EventStation
{
		...
		if (Type == Mode.Mach || Type == Mode.Chase)
		{
			...
			if (Type == Mode.Chase)
			{
				...
				if (!Damaged && HP <= 0)
				{
					...
					playerBase.Camera.CanCancelCinematic = true;
					playerBase.SetCameraParams(new CameraParameters(3, m_Camera, base.transform.position, ChaseCamPosition, CamTarget));
	...
	private void Camera()
	{
		if (Type != Mode.Chase || !Damaged)
		{
			...
			PlayerBase playerBase = Object.FindObjectOfType<PlayerBase>();
			playerBase.Camera.CanCancelCinematic = true;
			playerBase.SetCameraParams(new CameraParameters(3, m_Camera, base.transform.position, null, CamTarget));
			playerBase.Camera.UncancelableEvent = true;
		}
	}
