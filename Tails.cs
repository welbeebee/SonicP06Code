...
public class Tails : PlayerBase
{
	...
	public override void Awake()
	{
		...
		JumpWalkSpeed = Tails_Lua.c_jump_walk;
		JumpTopSpeed = Tails_Lua.c_jump_run;
	...
	private void StateJump()
	{
		...
		JumpAnimation = ((!(AirMotionVelocity.y > -3f)) ? 2 : ((Time.time - JumpTime > Tails_Lua.c_jump_time_min || JumpBounce) ? 1 : 0));
		...
		if (JumpAttackSphere(base.transform.position, 0.5f, base.transform.forward * _Rigidbody.velocity.magnitude, 1))
		{
			...
			HalveSinkJump = false;
			JumpAnimation = 1;
			AirMotionVelocity.y = Tails_Lua.c_jump_speed * 1.5f;
		}
	...
	private void StateFly()
	{
		...
		AttackSphere(base.transform.position + base.transform.up * 0.675f, 0.2f, base.transform.forward * CurSpeed + base.transform.forward * 20f, 1);
	...
	public override void FixedUpdate()
	{
		...
		if (Singleton<GameManager>.Instance.GameState != GameManager.State.Paused && StageManager.StageState != StageManager.State.Event)
		{
			if (!LockControls)
			{
				...
				{
					MaximumSpeed = (HasSpeedUp ? Tails_Lua.c_speedup_speed_max : (IsGrounded() ? TopSpeed : JumpTopSpeed));
				}
			}
			else
			{
	...
	public override void Update()
	{
		...
		if (Singleton<GameManager>.Instance.GameState != GameManager.State.Paused && Singleton<GameManager>.Instance.GameState != GameManager.State.Result && StageManager.StageState != StageManager.State.Event && !IsDead && PlayerState != State.Talk)
		{
			...
			{
				if (Time.time - AirThrowCD > Common_Lua.c_input_cd && Singleton<RInput>.Instance.P.GetButtonDown("Button X") && (CanAirThrow || (!CanAirThrow && Time.time - AirThrowTime > 0.3f)))
				{
					CanAirThrow = false;
			...
			if (PlayerState == State.TailSwipe && Time.time - SwipeTimer > Common_Lua.c_input_cd && Singleton<RInput>.Instance.P.GetButtonDown("Button A"))
			{
				StateMachine.ChangeState(StateJump);
			}
		}
		if (PlayerState == State.Fly || ((PlayerState == State.Ground || PlayerState == State.DashPanel || PlayerState == State.Path) && CurSpeed >= TopSpeed - 2f))
		{
			if (!PlayedFlySource)
			{
	...
	public override void UpdateAnimations()
	{
		...
		FlyAnimation.x = Mathf.MoveTowards(FlyAnimation.x, FlyReleasedKey ? 0f : 1f, Time.deltaTime * (FlyReleasedKey ? 3.5f : 4.5f));
