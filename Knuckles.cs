...
public class Knuckles : PlayerBase
{
	...
	private bool GlideReleasedKey;

	private float GlideLockTime;

	private float GlideKeyTime;
	...
	public override void Awake()
	{
		...
		JumpWalkSpeed = Knuckles_Lua.c_jump_walk;
		JumpTopSpeed = Knuckles_Lua.c_jump_run;
	...
	private void StateJump()
	{
		...
		JumpAnimation = ((!(AirMotionVelocity.y > -3f)) ? 2 : ((Time.time - JumpTime > Knuckles_Lua.c_jump_time_min || JumpBounce) ? 1 : 0));
		...
		if (JumpAttackSphere(base.transform.position, 0.5f, base.transform.forward * _Rigidbody.velocity.magnitude, 1))
		{
			...
			HalveSinkJump = false;
			JumpAnimation = 1;
			AirMotionVelocity.y = Knuckles_Lua.c_jump_speed * 1.5f;
		}
	...
	private void StateGlideStart()
	{
		...
		GlideSources[0].Play();
		GlideSources[1].Play();
		GlideSpeed = Mathf.Max(Knuckles_Lua.c_flight_acc, CurSpeed);
		GlideYGrav = 0f;
		AirMotionVelocity = _Rigidbody.velocity;
	...
	private void StateGlide()
	{
		...
		if (TargetDirection.magnitude != 0f)
		{
			GlideSpeed += Knuckles_Lua.c_flight_acc * Time.fixedDeltaTime * ((GlideSpeed > Knuckles_Lua.c_flight_speed_min) ? 1f : 2f);
		}
		...
		{
			GlideSpeed += Knuckles_Lua.c_flight_acc * Time.fixedDeltaTime * 2f;
		}
		GlideSpeed = Mathf.Clamp(GlideSpeed, 0f, GlideSpeedInc);
		GlideYGrav = ((AirMotionVelocity.y < 0f) ? Mathf.Lerp(GlideYGrav, 2f, Time.fixedDeltaTime * 0.6f) : 12.5f);
		CurSpeed = GlideSpeed;
		Vector3 vector = new Vector3(AirMotionVelocity.x, 0f, AirMotionVelocity.z);
		if (_Rigidbody.velocity.magnitude != 0f)
		{
		...
		if (AirMotionVelocity.y < 0f && GlideResetY)
		{
			GlideYGrav = 0f;
			GlideResetY = false;
		}
		...
		{
			GlideSpeed = Knuckles_Lua.c_flight_speed_min * 0.4f;
	...
	private void StateClimb()
	{
		...
		LeftStickX = Mathf.MoveTowards(LeftStickX, Singleton<RInput>.Instance.P.GetAxis("Left Stick X") * Knuckles_Lua.c_climb_speed, Time.fixedDeltaTime * 50f);
		LeftStickY = Mathf.MoveTowards(LeftStickY, Singleton<RInput>.Instance.P.GetAxis("Left Stick Y") * Knuckles_Lua.c_climb_speed, Time.fixedDeltaTime * 50f);
		Vector3 velocity = base.transform.right * LeftStickX + base.transform.up * LeftStickY;
		GeneralMeshRotation = Quaternion.LookRotation(ForwardMeshRotation, UpMeshRotation);
		base.transform.rotation = Quaternion.Lerp(base.transform.rotation, Quaternion.FromToRotation(-base.transform.forward, ClimbHit.normal) * base.transform.rotation, Time.deltaTime * 10f);
	...
	{
		PlayerState = State.ClimbUp;
		LockControls = Time.time - ClimbUpTime < 0.15f;
		CurSpeed = ClimbUpSpd;
		Vector3 vector = new Vector3(AirMotionVelocity.x, 0f, AirMotionVelocity.z);
		if (_Rigidbody.velocity.magnitude != 0f)
		{
	...
	private void StatePunchStart()
	{
		...
		PunchCount++;
		PunchSpeed = ((PunchCount != 3) ? TopSpeed : ((FinisherType == 0) ? TopSpeed : 0f)) * ((TargetDirection != Vector3.zero) ? 1f : 0.5f);
		PunchDamage = true;
		QueuedPress = false;
	...
	private void StatePunch()
	{
		...
		else
		{
			...
			{
				PunchSpeed = ((Time.time - PunchTimer < 0.5f) ? (TopSpeed * 2f) : Mathf.Lerp(PunchSpeed, 0f, Time.fixedDeltaTime * 25f));
	...
	private void StateUppercutStart()
	{
		...
		CurSpeed = TopSpeed * 1.5f * ((TargetDirection != Vector3.zero) ? 1f : 0.5f);
		AirMotionVelocity = _Rigidbody.velocity;
	...
	public override void FixedUpdate()
	{
		...
		if (Singleton<GameManager>.Instance.GameState != GameManager.State.Paused && StageManager.StageState != StageManager.State.Event)
		{
			if (!LockControls)
			{
				...
				{
					MaximumSpeed = (HasSpeedUp ? Knuckles_Lua.c_speedup_speed_max : (IsGrounded() ? TopSpeed : JumpTopSpeed));
				}
			}
			else
			{
	...
	public override void Update()
	{
		...
		if (Singleton<GameManager>.Instance.GameState != GameManager.State.Paused && Singleton<GameManager>.Instance.GameState != GameManager.State.Result && StageManager.StageState != StageManager.State.Event && !IsDead && PlayerState != State.Talk)
		{
			...
			if ((PlayerState == State.Jump || PlayerState == State.Air || PlayerState == State.SlowFall || PlayerState == State.BallFall || (PlayerState == State.Uppercut && Time.time - UppercutTime > 0.55f) || PlayerState == State.AfterHoming || (PlayerState == State.Spring && !LockControls) || (PlayerState == State.WideSpring && !LockControls) || (PlayerState == State.JumpPanel && !LockControls) || (PlayerState == State.RainbowRing && !LockControls)) && Singleton<RInput>.Instance.P.GetButtonDown("Button X"))
			{
				StateMachine.ChangeState(StateQuake);
			}
			if (PlayerState == State.Jump || PlayerState == State.Air || PlayerState == State.SlowFall || PlayerState == State.BallFall || (PlayerState == State.ClimbUp && Time.time - ClimbUpTime > 0.15f) || (PlayerState == State.Uppercut && Time.time - UppercutTime > 0.55f) || (PlayerState == State.Quake && QuakeState == 0) || PlayerState == State.AfterHoming || (PlayerState == State.Spring && !LockControls) || (PlayerState == State.WideSpring && !LockControls) || (PlayerState == State.JumpPanel && !LockControls) || (PlayerState == State.RainbowRing && !LockControls))
			{
				if (!LockControls && !GlideReleasedKey && !Singleton<RInput>.Instance.P.GetButton("Button A"))
				{
					GlideReleasedKey = true;
				}
				if (((PlayerState == State.Jump && ReleasedKey) || (PlayerState != State.Jump && GlideReleasedKey)) && Singleton<RInput>.Instance.P.GetButton("Button A") && ((CanGlide && Time.time - GlideKeyTime > Common_Lua.c_input_cd) || (!CanGlide && Time.time - GlideLockTime > Knuckles_Lua.c_flight_cd)))
				{
					CanGlide = false;
			...
			{
				GlideReleasedKey = false;
				GlideKeyTime = Time.time;
			}
			...
			if (PlayerState == State.Climb && Singleton<RInput>.Instance.P.GetButtonDown("Button A"))
			{
				...
				CurSpeed = TopSpeed / 1.5f;
				StateMachine.ChangeState(StateJump);
			}
			if (PlayerState == State.ClimbUp && Time.time - ClimbUpTime > 0.15f && Singleton<RInput>.Instance.P.GetButtonDown("Button X"))
			{
				StateMachine.ChangeState(StateQuake);
			}
			if (PlayerState == State.Punch)
			{
				...
				if (Time.time - PunchTimer > Common_Lua.c_input_cd && PunchCount < 3 && Singleton<RInput>.Instance.P.GetButtonDown("Button A"))
				{
					StateMachine.ChangeState(StateJump);
				}
			}
			...
			if (PlayerState == State.Quake && QuakeState == 1)
			{
