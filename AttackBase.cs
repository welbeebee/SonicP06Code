...
public class AttackBase : MonoBehaviour
{
	...
	internal List<Collider> ChaosBlastPass = new List<Collider>();

	internal LayerMask HomingBlock_Mask => LayerMask.GetMask("Default", "DefaultObject", "PlayerCollision", "PlayerCollisionObject", "BreakableObj", "InvisibleCollision", "Object/PlayerOnlyCol");
	...
	public bool AttackSphere_Dir(float Radius, float SpreadMultp, int Damage = 1, string Type = "")
	{
		...
		for (int i = 0; i < array.Length; i++)
		{
			if (!(array[i] != null) || (!(Type != "ChaosBlast") && (!(Type == "ChaosBlast") || ChaosBlastPass.Contains(array[i]))))
			{
				continue;
			}
