...
public class eGuardian : EnemyBase
{
	...
	private void StateFreezeStart()
	{
		...
		Animation = 17;
		StateVar[0] = 0;
		MainCollider.center = new Vector3(0f, 1.25f, 0f);
		MainCollider.size = new Vector3(3f, 2.5f, 3f);
	}
	...
	private void CreateArms()
	{
		...
		if (!Arm[0])
		{
			...
			Arm[0].GetComponent<ReturnTriggerMessage>().ReturnTo = base.transform;
		}
		if (!Arm[1])
		{
			...
			Arm[1].GetComponent<ReturnTriggerMessage>().ReturnTo = base.transform;
		}
	}
