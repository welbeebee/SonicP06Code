...
public class Bike : VehicleBase
{
	...
	public override void Update()
	{
		...
		for (int i = 0; i < BoosterFireFX.Length; i++)
		{
			ParticleSystem.EmissionModule emission = BoosterFireFX[i].emission;
		...
		for (int j = 0; j < 2; j++)
		{
			ParticleSystem.EmissionModule emission2 = WheelSmokeDirtFX[j].emission;
			emission2.enabled = Wheels[0].isGrounded && hit2.collider.transform.tag == "Dirt";
			emission2.rateOverTime = num;
		}
		for (int k = 0; k < 3; k++)
		{
			ParticleSystem.EmissionModule emission3 = WheelSmokeGrassFX[k].emission;
			emission3.enabled = Wheels[0].isGrounded && hit2.collider.transform.tag == "Grass";
			emission3.rateOverTime = num;
		}
		for (int l = 2; l < 4; l++)
		{
			ParticleSystem.EmissionModule emission4 = WheelSmokeDirtFX[l].emission;
			emission4.enabled = Wheels[1].isGrounded && hit3.collider.transform.tag == "Dirt";
			emission4.rateOverTime = num;
		}
		for (int m = 3; m < 6; m++)
		{
			ParticleSystem.EmissionModule emission5 = WheelSmokeGrassFX[m].emission;
			emission5.enabled = Wheels[1].isGrounded && hit3.collider.transform.tag == "Grass";
			emission5.rateOverTime = num;
		}
		ParticleSystem.EmissionModule emission6 = BrakeSmokeConcreteFX.emission;
		emission6.enabled = Wheels[1].isGrounded && hit3.collider.transform.tag == "Normal" && IsMounted && ((Singleton<RInput>.Instance.P.GetButton("Button X") && Wheels[1].rpm == 0f) || ((hit.forwardSlip >= SlipLimit || hit.forwardSlip <= (0f - SlipLimit) * ((Singleton<RInput>.Instance.P.GetButton("Button X") && Speed > 0f) ? 0.25f : 1f) || hit.sidewaysSlip >= SlipLimit * 0.25f || hit.sidewaysSlip <= (0f - SlipLimit) * 0.25f) && CurrentTorque >= 0f));
		for (int n = 0; n < BrakeSmokeDirtFX.Length; n++)
		{
			ParticleSystem.EmissionModule emission7 = BrakeSmokeDirtFX[n].emission;
			emission7.enabled = Wheels[1].isGrounded && hit3.collider.transform.tag == "Dirt" && IsMounted && ((Singleton<RInput>.Instance.P.GetButton("Button X") && Wheels[1].rpm == 0f) || ((hit.forwardSlip >= SlipLimit || hit.forwardSlip <= (0f - SlipLimit) * ((Singleton<RInput>.Instance.P.GetButton("Button X") && Speed > 0f) ? 0.25f : 1f) || hit.sidewaysSlip >= SlipLimit * 0.25f || hit.sidewaysSlip <= (0f - SlipLimit) * 0.25f) && CurrentTorque >= 0f));
		}
		for (int num2 = 0; num2 < BrakeSmokeGrassFX.Length; num2++)
		{
			ParticleSystem.EmissionModule emission8 = BrakeSmokeGrassFX[num2].emission;
			emission8.enabled = Wheels[1].isGrounded && hit3.collider.transform.tag == "Grass" && IsMounted && ((Singleton<RInput>.Instance.P.GetButton("Button X") && Wheels[1].rpm == 0f) || ((hit.forwardSlip >= SlipLimit || hit.forwardSlip <= (0f - SlipLimit) * ((Singleton<RInput>.Instance.P.GetButton("Button X") && Speed > 0f) ? 0.25f : 1f) || hit.sidewaysSlip >= SlipLimit * 0.25f || hit.sidewaysSlip <= (0f - SlipLimit) * 0.25f) && CurrentTorque >= 0f));
		}
		Renderer.GetPropertyBlock(PropBlock, 4);
		...
		if (IsMounted)
		{
			if ((bool)PM)
			{
				...
				float num4 = Mathf.Lerp(0.5f, 1.5f, ScaledVelSpd() / Vehicle_Param_Bike_Lua.f_MaxSpeed);
				PM.Base.CamOffset.x = Mathf.Lerp(PM.Base.CamOffset.x, (VehicleState == 1) ? Singleton<RInput>.Instance.P.GetAxis("Left Stick X") : 0f, num3);
				PM.Base.CamOffset.y = Mathf.Lerp(PM.Base.CamOffset.y, Common_Lua.c_camera.y + ((Wheels[0].isGrounded && Wheels[1].isGrounded) ? 0.1f : (-0.25f)), Time.deltaTime * 2f);
				PM.Base.CamOffset.z = Mathf.Lerp(PM.Base.CamOffset.z, UseBooster ? 2.5f : num4, UseBooster ? (Time.deltaTime * 2f) : num3);
			}
			if (Singleton<RInput>.Instance.P.GetButtonDown("Button A") && !UseBooster && Time.time - BoosterTime > Vehicle_Param_Bike_Lua.f_boost_hold_time)
			{
				if (ButtonCooler > 0f && ButtonCount == 1 && !Singleton<RInput>.Instance.P.GetButton("Button X"))
				{
					...
					float num5 = Mathf.Lerp(1f, 0f, Speed / Vehicle_Param_Bike_Lua.f_MaxSpeed * 1.25f);
					_Rigidbody.AddForce(base.transform.forward * (Vehicle_Param_Bike_Lua.f_boost_power_bias * num5), ForceMode.VelocityChange);
			...
			CurrentTopSpeed = ((!UseBooster) ? Mathf.MoveTowards(CurrentTopSpeed, Vehicle_Param_Bike_Lua.f_MaxSpeed, Time.deltaTime * 7.5f) : (Vehicle_Param_Bike_Lua.f_MaxSpeed * 1.25f));
			...
			if (IsShooting)
			{
				...
				if (CurAmmo == 0)
				{
					for (int num6 = 0; num6 < GunSmokeFX.Length; num6++)
					{
						GunSmokeFX[num6].Play();
					}
				}
				IsShooting = false;
			}
	...
	private void FixedUpdate()
	{
		...
		_Rigidbody.drag = ((!Singleton<RInput>.Instance.P.GetButton("Button X") && !Singleton<RInput>.Instance.P.GetButton("Button A") && Wheels[0].isGrounded && Wheels[1].isGrounded) ? 0.15f : 0f);
		float num = Mathf.Clamp(SqrMag() / Vehicle_Param_Bike_Lua.f_MaxSpeed * 2.5f, 0f - SteerXAxis, SteerXAxis);
		Quaternion quaternion = Quaternion.AngleAxis(SteerYAxis * Time.fixedDeltaTime * 75f, base.transform.right);
