...
public class Aqa_Balancer : ObjectBase
{
	...
	private void StateBalancerStart()
	{
		...
		PM.transform.rotation = Quaternion.LookRotation(PM.transform.forward.MakePlanar());
	...
	private void FixedUpdate()
	{
		...
		if ((bool)PM)
		{
			...
			if (PM.Base.GetState() == "Balancer" && Singleton<GameManager>.Instance.GameState != GameManager.State.Paused)
			{
				...
				if (PM.Base.TargetDirection != Vector3.zero && !flag)
				{
					...
					PM.transform.rotation = Quaternion.Slerp(PM.transform.rotation, b, PM.Base.RotOverSpdCurve.Evaluate(PM.Base.CurSpeed) * Time.fixedDeltaTime);
				}
			}
			if (PM.Base.TargetDirection != Vector3.zero && !flag)
			{
			...
			if (PM.Base.TurnDeltaAbs() > Common_Lua.c_turn_speed_damp && Speed > 0f)
			{
				Speed -= Deceleration * Time.fixedDeltaTime * 2f;
			}
			ForwardMove = PM.transform.forward;
		}
		else
		{
		...
		Rigidbody.velocity = ForwardMove * Speed;
	}
	...
	private void OnBounce(ContactPoint[] Contacts, bool Damage = false)
	{
		for (int i = 0; i < Contacts.Length; i++)
		{
			...
			if ((bool)PM)
			{
				PM.transform.forward = (Quaternion.AngleAxis(180f, contactPoint.normal) * PM.transform.forward * -1f).MakePlanar();
			}
			else
			{
				ForwardMove = (Quaternion.AngleAxis(180f, contactPoint.normal) * ForwardMove * -1f).MakePlanar();
			}
			if (Damage)
			{
