...
public class UI : UIBase
{
	...
	private void Update()
	{
		...
		if (Singleton<Settings>.Instance.settings.DisplayType == 0)
		{
			ActionGaugeObj.anchoredPosition = Vector3.Lerp(ActionGaugeObj.anchoredPosition, new Vector3(ActionGaugeObj.anchoredPosition.x, (!UseGauge) ? 0f : ((!(GaugeModule == "gauge_module_sonic")) ? ((GaugeModule != "") ? 360f : 200f) : ((PM.sonic.GemSelector == 0) ? 200f : 360f))), Time.deltaTime * 20f);
		}
		else
		{
			ActionGaugeObj.anchoredPosition = Vector3.Lerp(ActionGaugeObj.anchoredPosition, new Vector3(ActionGaugeObj.anchoredPosition.x, (!UseGauge) ? 0f : ((!(GaugeModule == "gauge_module_sonic")) ? ((GaugeModule != "") ? 346.5f : 200f) : ((PM.sonic.GemSelector == 0) ? 200f : 346.5f))), Time.deltaTime * 20f);
		}
		...
		{
			if (LevelAnimator.GetBool("Is Upgrade Limit"))
			{
				LevelAnimator.SetBool("Is Upgrade Limit", value: false);
			}
			if (LevelAnimator.GetBool("Is Highlight"))
			{
				LevelAnimator.SetBool("Is Highlight", value: false);
			}
		}
		...
		if (UseGauge)
		{
			...
			if (ActionDisplay >= MaxActionGauge)
			{
				...
				{
					PM.Base.Audio.PlayOneShot(actionGaugeMax, PM.Base.Audio.volume * 0.5f);
			...
			{
				if (!PM.Base.IsGrounded() && PM.Base.GetState() != "Path" && PM.Base.GetState() != "Grinding" && PM.Base.GetState() != "WaterSlide")
				{
			...
			{
				if (PM.princess.UsingShield || (!PM.Base.IsGrounded() && PM.Base.GetState() != "Path"))
				{
					GaugeWaitToRefill = Time.time;
				...
				{
					ReplenishActionGauge(flag && (PM.Base.IsGrounded() || PM.Base.GetState() == "Path" || PM.Base.GetState() == "Grinding"));
				}
			}
			...
			if (GaugeModule == "gauge_module_silver")
			{
				...
				if (PM.Base.GetState() == "Levitate" || PM.Base.GetState() == "PsychicShot" || PM.Base.GetState() == "Upheave" || PM.Base.GetState() == "Dash" || (!PM.Base.IsGrounded() && PM.Base.GetState() != "Path" && PM.Base.GetState() != "Grinding") || PM.silver.UsingPsychokinesis || PM.silver.PickedObjects.Count != 0)
				{
					GaugeWaitToRefill = Time.time;
			...
			if (GaugeModule == "gauge_module_tails")
			{
				...
				{
					ReplenishActionGauge(flag && PM.Base.GetState() != "AerialTailSwipe" && ((!PM.Base.IsGrounded() && PM.tails.CanRegenFly) || PM.Base.IsGrounded() || PM.Base.GetState() == "Path" || PM.Base.GetState() == "Grinding"));
				}
			}
			if (GaugeModule == "gauge_module_omega")
			{
				if (PM.Base.GetState() == "Hover" || (!PM.Base.IsGrounded() && PM.Base.GetState() != "Path" && PM.Base.GetState() != "Grinding"))
				{
					GaugeWaitToRefill = Time.time;
			...
			{
				if (PM.Base.GetState() == "Boost" || (!PM.Base.IsGrounded() && PM.Base.GetState() != "Path" && PM.Base.GetState() != "Grinding" && PM.Base.GetState() != "WaterSlide"))
				{
					GaugeWaitToRefill = Time.time;
		...
		if (UseRadar)
		{
			...
			if ((bool)RadarTarget1 && !RadarTarget1.Obtained)
			{
				...
				{
					Singleton<AudioManager>.Instance.PlayClip(RadarBeep, 1.25f);
	...
	public float StartMessageBox(string[] Texts, string[] Sounds, float Duration = 3f)
	{
		...
		if (Singleton<Settings>.Instance.settings.TextBoxType != 3)
		{
	...
	{
		if (Singleton<Settings>.Instance.settings.CheckpointType != 1)
		{
