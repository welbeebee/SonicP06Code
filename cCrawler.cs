	...
	private void StateShootBomb()
	{
		if (StateVar[0] == 0)
		{
			...
			if (Time.time - StateTime[0] > 1f && AttackCount <= MaxAttack && StateVar[1] == 0)
			{
				...
				gameObject.GetComponent<Rigidbody>().AddForce((TargetPosition - BombPoint.position).normalized * ((EnemyType == Type.cCrawler) ? 20f : 24f) + Mesh.up * -2.5f, ForceMode.Impulse);
	...
	private void OnHit(HitInfo HitInfo)
	{
		...
		else
		{
			...
			if (CurHealth > 0)
			{
				...
				if (!ParalysisEffect && EnemyState != 0 && EnemyState != State.DivingAttack && !IsPsychokinesis && !PsychoThrown)
				{
