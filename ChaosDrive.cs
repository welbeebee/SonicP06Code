...
public class ChaosDrive : ObjectBase
{
	...
	private void FixedUpdate()
	{
		...
		if (!Grabbed)
		{
			float num2 = Mathf.Lerp(7.5f, 17.5f, Mathf.Clamp01(num));
			float num3 = Mathf.Min(num * num2, 60f);
			base.transform.forward = Vector3.Lerp(base.transform.forward, (Target.transform.position + Target.transform.up * 0.25f - base.transform.position).normalized, Time.fixedDeltaTime * num3);
			float num4 = Mathf.Min(num * 20f, 20f) + Mathf.Min(num * 5f, 30f);
			_Rigidbody.MovePosition(base.transform.position + base.transform.forward * num4 * Time.fixedDeltaTime);
			Mesh.RotateAround(Mesh.position, Mesh.right, 30f * num4 * Time.fixedDeltaTime);
		}
