...
public class SonicEffects : EffectsBase
{
	...
	public Color SuperUpgradeColor;

	public Color SuperOutlineColor;
	...
	public ParticleSystem jumpDashStartParticles;
	...
	public ParticleSystem SuperJumpDashStartFX;
	...
	private float SpindashBlinkTime;
	...
	public override void Update()
	{
		...
		if (Singleton<Settings>.Instance.settings.SpinEffect != 0 && Singleton<Settings>.Instance.settings.SpinEffect != 3)
		{
			...
			{
				SpindashBlink += Time.deltaTime * 30f;
				SpindashBlinkTime += Time.deltaTime * 2f;
				if (SpindashBlink >= 1f)
				{
					SpindashBlink = 0f;
				}
				if (SpindashBlinkTime >= 1f)
				{
					SpindashBlinkTime = 0f;
				}
				SpinDashBallRenderer.enabled = !PM.sonic.IsSuper && (SpindashBlinkTime > 0.5f || (SpindashBlink <= 0.25f && SpindashBlinkTime < 0.5f));
				SuperSpinDashBallRenderer.enabled = PM.sonic.IsSuper && (SpindashBlinkTime > 0.5f || (SpindashBlink <= 0.25f && SpindashBlinkTime < 0.5f));
			}
			else
			{
		...
		for (int j = 0; j < SuperRenderers.Length; j++)
		{
			...
			PropBlock.SetColor("_OutlineColor", SuperOutlineColor);
			PropBlock.SetFloat("_OutlinePulseSpd", 0f);
		...
		if (PM.sonic.Upgrades.Renderers != null && Singleton<Settings>.Instance.settings.UpgradeModels == 0)
		{
			...
			for (int k = 0; k < PM.sonic.Upgrades.Renderers.Count; propBlock4.SetFloat("_OutlineInt", value2), PM.sonic.Upgrades.Renderers[k].SetPropertyBlock(PropBlock), k++)
			{
				...
				{
					PropBlock.SetColor("_ExtFresColor", SuperUpgradeColor);
					PropBlock.SetColor("_ExtGlowColor", SuperUpgradeColor);
					PropBlock.SetColor("_OutlineColor", SuperOutlineColor);
				}
				PropBlock.SetFloat("_ExtPulseSpd", 1f);
				PropBlock.SetFloat("_ExtFresPower", 1f);
	...
	public void PlayJumpDashStartFX(Vector3 Direction)
	{
		if (!PM.sonic.IsSuper)
		{
			jumpDashStartParticles.transform.forward = Direction;
			jumpDashStartParticles.Play();
		}
		else
		{
			SuperJumpDashStartFX.transform.forward = Direction;
			SuperJumpDashStartFX.Play();
		}
	}
	...
	public void CreateTransformFX(bool Super = true)
	{
		...
		if (Singleton<Settings>.Instance.settings.SpinEffect == 1 && SuperJumpballRenderer.enabled)
		{
			SuperJumpballRenderer.enabled = false;
		}
	}
