...
public class SonicNew : PlayerBase
{
	...
	private float LookAtVolcanoTime;

	private float LookAtVolcanoAnim;
	...
	public override void Awake()
	{
		...
		JumpWalkSpeed = Sonic_New_Lua.c_jump_walk;
		JumpTopSpeed = Sonic_New_Lua.c_jump_run;
	...
	private void StateJump()
	{
		...
		JumpAnimation = ((!(AirMotionVelocity.y > -3f)) ? 2 : ((Time.time - JumpTime > Sonic_New_Lua.c_jump_time_min || JumpBounce) ? 1 : 0));
		if (!Singleton<RInput>.Instance.P.GetButton("Button A"))
		{
			ReleasedKey = true;
		}
		if (Singleton<GameManager>.Instance.GameState != GameManager.State.Paused && !ReleasedKey && Singleton<RInput>.Instance.P.GetButton("Button A") && Time.time - JumpTime < (JumpBounce ? 0.25f : ((!PurpleGemJump && !HalveSinkJump) ? 0.7f : 0.45f)))
		{
			float num = ((!IsSuper || HUD.ActiveGemLevel[GemSelector] <= 0) ? ((!HalveSinkJump) ? 4.25f : 3f) : ((!HalveSinkJump) ? 6.25f : 5f));
			AirMotionVelocity += Vector3.up * num * Time.fixedDeltaTime * ((!JumpBounce) ? 4f : 6.75f);
		}
		if (JumpAttackSphere(base.transform.position, 0.5f, base.transform.forward * _Rigidbody.velocity.magnitude, 1))
		{
			...
			HalveSinkJump = false;
			JumpAnimation = 1;
			AirMotionVelocity.y = Sonic_New_Lua.c_jump_speed * 1.5f;
		}
	...
	private void StateJumpDashStart()
	{
		...
		SonicEffects.PlayJumpDashStartFX(base.transform.forward);
	}

	private void StateJumpDash()
	{
		PlayerState = State.JumpDash;
		PlayAnimation("Homing Attack", "On Homing Attack");
		_ = (Time.time - JumpDashStartTime) / JumpDashLength;
		GeneralMeshRotation = Quaternion.LookRotation(ForwardMeshRotation, UpMeshRotation);
		base.transform.rotation = Quaternion.FromToRotation(base.transform.up, Vector3.up) * base.transform.rotation;
		JumpDashSpeed -= Sonic_New_Lua.c_homing_brake * 0.65f * Time.fixedDeltaTime;
		...
		{
			AirMotionVelocity.y -= 19.62f * Time.fixedDeltaTime;
		}
		AirMotionVelocity.y = LimitVel(AirMotionVelocity.y);
		_Rigidbody.velocity = AirMotionVelocity;
	...
	private void StateLightDash()
	{
		...
		if (splineTime < MinTime || splineTime > MaxTime)
		{
			if (IsGrounded())
			{
				CurSpeed = (TopSpeed + Sonic_New_Lua.c_lightdash_speed) / 1.75f;
				StateMachine.ChangeState(StateGround);
			}
			else
			{
	...
	private void StateHomingStart()
	{
		...
		SonicEffects.PlayJumpDashStartFX(HomingDirection);
	}
	...
	private void StateAfterHoming()
	{
		...
		if (Time.time - AHEnterTime <= 0.25f || (Time.time - AHEnterTime <= 0.5f && HAPressed))
		{
	...
	private void StateKick()
	{
		...
		{
			FirstKickSpeed -= TopSpeed * Time.fixedDeltaTime;
		}
	...
	private void StateSlide()
	{
		...
		SlideSpeed = Mathf.Clamp(SlideSpeed, Sonic_New_Lua.c_sliding_speed_min, TopSpeed * 1.5f);
		CurSpeed = SlideSpeed;
		_Rigidbody.velocity = Vector3.ProjectOnPlane(base.transform.forward, RaycastHit.normal) * CurSpeed;
	...
	private void StateTrickJump()
	{
		...
		if (Singleton<GameManager>.Instance.GameState != GameManager.State.Paused && !ReleasedTrickKey && Singleton<RInput>.Instance.P.GetButton("Button A") && Time.time - TrickJumpTime < ((!JumpBounce) ? 0.8f : 0.25f))
		{
	...
	private void StateSpinDashStart()
	{
		...
		SpindashSpd = Mathf.Min(Mathf.Max(CurSpeed, WalkSpeed * 6f), Sonic_New_Lua.c_spindash_spd * 1.5f);
	...
	private void StateSpinDash()
	{
		...
		else
		{
			...
			CurSpeed = Mathf.Clamp(CurSpeed, 0f, TopSpeed * 2f);
			_Rigidbody.velocity = Vector3.ProjectOnPlane(base.transform.forward, RaycastHit.normal) * CurSpeed;
			if (FrontalCollision)
			{
				StateMachine.ChangeState(StateGround);
	...
	private void StateWaterSlideStart()
	{
		...
		WSpeed = Mathf.Min(CurSpeed, TopSpeed * 1.5f);
		WSTime = 0f;
	...
	private void StateTransform()
	{
		...
		if (Time.time - TransformTimer > 1.5f && !HasTransformed)
		{
			...
			AttackSphere_Dir(base.transform.position + base.transform.up * 0.25f, 5f, 10f, 10);
	...
	public override void FixedUpdate()
	{
		...
		if (Singleton<GameManager>.Instance.GameState != GameManager.State.Paused && StageManager.StageState != StageManager.State.Event)
		{
			if (!LockControls)
			{
				...
				if (UsingBlueGem)
				{
					MaximumSpeed = TopSpeed + Sonic_New_Lua.c_custom_action_machspeed_acc;
					CurSpeed = TopSpeed + Sonic_New_Lua.c_custom_action_machspeed_acc;
				}
				else if (WalkSwitch)
				{
				...
				{
					MaximumSpeed = (HasSpeedUp ? (Sonic_New_Lua.c_speedup_speed_max * ((!flag) ? 1f : 1.3f)) : (IsGrounded() ? (TopSpeed * ((!flag) ? 1f : 1.3f)) : (JumpTopSpeed * ((!flag) ? 1f : 1.3f))));
				}
			}
			else
			{
	...
	public override void Update()
	{
		...
		if (IsGrounded() && AfterHomingTrick != 0)
		{
			AfterHomingTrick = 0;
		}
		...
		{
			if (PlayerState != State.Orca && PlayerState != State.Result && PlayerState != State.Transform && PlayerState != State.Cutscene && !UsingGreenGem && !UsingWhiteGem && !IsSuper && GemData.ObtainedGems != null && GemData.ObtainedGems.Count > 1)
			{
			...
			{
				if (Time.time - SpindashTime > Common_Lua.c_input_cd && Singleton<RInput>.Instance.P.GetButtonDown("Button A"))
				{
					StateMachine.ChangeState(StateJump);
				}
				...
				{
					SpinDashState = 1;
					Audio.PlayOneShot((Singleton<Settings>.Instance.settings.SpinEffect == 0 || Singleton<Settings>.Instance.settings.SpinEffect == 3) ? SpinDashShoot : ((Singleton<Settings>.Instance.settings.SpinEffect == 1) ? SpinDashShootAdventure : SpinDashShootAdventure2), Audio.volume);
			...
			if ((IsGrounded() || PlayerState == State.Grinding) && !CanFallJumpdash)
			{
			...
			{
				if (Time.time - KickTime > Common_Lua.c_input_cd && Singleton<RInput>.Instance.P.GetButtonDown("Button A"))
				{
					JumpType = 1;
			...
			{
				if (Time.time - SlideEndTimer > Common_Lua.c_input_cd && Singleton<RInput>.Instance.P.GetButtonDown("Button A"))
				{
					JumpType = 0;
					StateMachine.ChangeState(StateTrickJump);
				}
			...
			if (UsingGreenGem && Time.time - TornadoTimer > Common_Lua.c_input_cd && Singleton<RInput>.Instance.P.GetButtonDown("Button A"))
			{
				if (IsGrounded())
				{
					StateMachine.ChangeState(StateJump);
			...
			{
				if (PlayerState != State.Orca && PlayerState != State.Result && PlayerState != State.Cutscene && Singleton<RInput>.Instance.P.GetButtonDown("Right Trigger") && HUD.ActionDisplay > 0f)
				{
					UsingRedGem = true;
			...
			{
				float num4 = Sonic_New_Lua.c_blue * ((HUD.ActiveGemLevel[GemSelector] == 0) ? 1.25f : ((HUD.ActiveGemLevel[GemSelector] == 1) ? 1f : 0.75f));
				if (!LockControls && Singleton<RInput>.Instance.P.GetButtonDown("Right Trigger") && !UsingBlueGem && HUD.ActionDisplay >= num4)
				{
					UsingBlueGem = true;
					MachTimer = Time.time;
					Animator.SetTrigger("Additive Idle");
					SonicEffects.CreateMachSpeedFX(HUD.ActiveGemLevel[GemSelector]);
					HUD.DrainActionGauge(num4);
				}
			}
			else
			{
			...
			if (ActiveGem == Gem.Yellow)
			{
				if (PlayerState != State.Orca && PlayerState != State.Result && PlayerState != State.Cutscene && Singleton<RInput>.Instance.P.GetButtonDown("Right Trigger"))
				{
					...
					if (UsingYellowGem)
					{
						float num5 = Sonic_New_Lua.c_yellow * ((HUD.ActiveGemLevel[GemSelector] == 0) ? 1f : ((HUD.ActiveGemLevel[GemSelector] == 1) ? 0.75f : 0.5f));
						if (HUD.ActionDisplay > num5)
						{
							AddThunderShield();
							RemoveShield();
						}
			...
			{
				if (PlayerState != State.Orca && PlayerState != State.Result && PlayerState != State.Cutscene && PlayerState != State.GunDriveMove && Singleton<RInput>.Instance.P.GetButtonDown("Right Trigger") && HUD.ActionDisplay > 0f)
				{
					UsingPurpleGem = true;
	...
	public void LookAtVolcano()
	{
		IsLookAtVolcano = true;
		LookAtVolcanoTime = Time.time;
	}
	...
	public override void UpdateAnimations()
	{
		...
		if (IsLookAtVolcano && Time.time - LookAtVolcanoTime > 2.5f)
		{
			IsLookAtVolcano = false;
		}
		LookAtVolcanoAnim = Mathf.Lerp(LookAtVolcanoAnim, IsLookAtVolcano ? 1f : 0f, Time.deltaTime * 5f);
		Animator.SetFloat("Look At Volcano Anim", LookAtVolcanoAnim);
	...
	{
		if (PlayerState != State.Balancer && PlayerState != State.Result && PlayerState != State.Death && PlayerState != State.FallDeath && PlayerState != State.DrownDeath && PlayerState != State.SnowBallDeath)
		{
