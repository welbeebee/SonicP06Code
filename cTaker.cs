...
public class cTaker : EnemyBase
{
	...
	private void StateGlidingAttack()
	{
		...
		{
			if (Physics.Raycast(base.transform.position + base.transform.up * 0.4f, Mesh.forward, out var hitInfo, 0.75f, base.FrontalCol_Mask) && hitInfo.transform.GetComponent<Collider>() != GetComponent<Collider>())
			{
	...
	{
		if (!Destroyed && (CreatureMode != 0 || ChaseCanDie))
		{
			if ((CreatureMode != Mode.Fix_Bomb || CreatureMode != 0) && Reloaded)
			{
				DestroyBomb();
	...
	{
		if (HitTimer <= GetHitTimer(HitInfo) || (CreatureMode == Mode.Chase_Bomb && !ChaseCanDie))
		{
			return;
		}
		if ((CreatureMode != Mode.Fix_Bomb || CreatureMode != 0) && Reloaded)
	...
	public override void OnCollisionEnter(Collision collision)
	{
		...
		if (CreatureMode == Mode.Chase_Bomb && (bool)PathSpline && (collision.gameObject.layer == LayerMask.NameToLayer("PlayerCollision") || collision.gameObject.layer == LayerMask.NameToLayer("Default") || collision.gameObject.layer == LayerMask.NameToLayer("Ignore Raycast")))
		{
