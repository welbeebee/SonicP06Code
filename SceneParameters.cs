...
public class SceneParameters : MonoBehaviour
{
	...
	private void Update()
	{
		...
		{
			MainLightComp.color = Color.Lerp(MainLightComp.color, (LightChange && !LightAnimationChange) ? MainChangeColor : OrigMainColor, Time.deltaTime);
			MainLightComp.intensity = Mathf.Lerp(MainLightComp.intensity, LightChange ? MainChangeInt : OrigMainInt, Time.deltaTime);
			MainLightComp.transform.forward = Vector3.Lerp(MainLightComp.transform.forward, LightChange ? MainChangeDir : OrigMainDir, Time.deltaTime);
			SubLightComp.color = Color.Lerp(SubLightComp.color, (LightChange && !LightAnimationChange) ? SubChangeColor : OrigSubColor, Time.deltaTime);
			SubLightComp.intensity = Mathf.Lerp(SubLightComp.intensity, LightChange ? SubChangeInt : OrigSubInt, Time.deltaTime);
			SubLightComp.transform.forward = Vector3.Lerp(SubLightComp.transform.forward, LightChange ? SubChangeDir : OrigSubDir, Time.deltaTime);
			RenderSettings.ambientLight = Color.Lerp(RenderSettings.ambientLight, (LightChange && !LightAnimationChange) ? AmbChangeColor : OrigAmbColor, Time.deltaTime);
		}
