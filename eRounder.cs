...
public class eRounder : EnemyBase
{
	...
	private void StateGuard()
	{
		...
		Mesh.localRotation = FaceTarget(TargetPosition, Mesh.localRotation, Inverse: false, 1.5f, Smooth: true);
		if (Time.time > StateSound && !PlayedSounds)
		{
			PlayedSounds = true;
	...
	private void StateChargeAttack()
	{
		if (Time.time > TimeBeforeCharge)
		{
			...
			if (TimeAfterCharge < 1.1f)
			{
				...
				if (Physics.Raycast(base.transform.position + base.transform.up * 1.25f, Mesh.forward, out var hitInfo2, 0.75f, base.FrontalCol_Mask) && hitInfo2.transform.GetComponent<Collider>() != GetComponent<Collider>())
				{
