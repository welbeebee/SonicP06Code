...
public class eGunnerFly : EnemyBase
{
	...
	private bool ChaseCanDie;

	private bool ChaseContinue;
	...
	private void Start()
	{
		...
		if (RobotMode == Mode.Chase)
		{
			...
			if ((bool)GameObject.Find("Stage/Splines/" + base.ActionPath))
			{
				PathSpline = GameObject.Find("Stage/Splines/" + base.ActionPath).GetComponent<BezierCurve>();
			}
			StartTime = Time.time;
		}
		else
	...
	private void MovePath(float _Dist, float _WaitTime)
	{
		ChaseDist = _Dist;
		ChaseWaitTime = _WaitTime;
	}

	private new void ActionPath(float _Dist)
	{
		ChaseDist = _Dist;
	}
	...
	public override void FixedUpdate()
	{
		...
		if ((bool)PathSpline)
		{
			...
			float b = Mathf.Lerp(105f, 0.5f, num2 / (10f + ChaseDist));
			ChaseAccel = Mathf.Lerp(ChaseAccel, 5f, Time.fixedDeltaTime);
			FlySpeed = Mathf.Lerp(FlySpeed, b, Time.fixedDeltaTime * ChaseAccel);
			ActionSplineTime += FlySpeed / PathSpline.Length() * Time.fixedDeltaTime;
	...
	private void StateFlyStart()
	{
		...
		if (RobotMode == Mode.Chase)
		{
			...
			StateTime[0] = Time.time + ChaseWaitTime;
		}
		else
		{
	...
	private void StateFly()
	{
		if (RobotMode == Mode.Chase)
		{
			if (!ChaseContinue && Time.time > StateTime[0])
			{
				MovePath(7.875f, 1f);
				StateTime[0] = Time.time + ChaseWaitTime;
				ChaseLoopActions++;
				if (ChaseLoopActions > 3)
				{
					ChaseLoopActions = 0;
				}
				ChaseContinue = true;
			}
			if (ChaseContinue && Time.time > StateTime[0] && GetTarget(Mesh.forward))
			{
	...
	private void StateShootVulcan()
	{
		...
		{
			TargetPosition = Target.transform.position + Target.transform.up * 0.25f;
			Mesh.localRotation = FaceTarget(TargetPosition, Mesh.localRotation, Inverse: false, 2f, Smooth: true);
		}
		...
		if (RobotMode == Mode.Chase)
		{
			Mesh.localRotation = Quaternion.RotateTowards(Mesh.localRotation, Quaternion.identity, 6f);
		}
	...
	private void ManageJetFire()
	{
		FireFX[0].SetActive(RobotMode != Mode.Chase);
		FireFX[1].SetActive(RobotMode == Mode.Chase);
	}
	...
	{
		if (!Destroyed && (RobotMode != 0 || ChaseCanDie))
		{
	...
	{
		if (HitTimer <= GetHitTimer(HitInfo) || (RobotMode == Mode.Chase && !ChaseCanDie))
		{
			return;
		}
		if (CurHealth > 0)
	...
	public override void OnCollisionEnter(Collision collision)
	{
		...
		if (RobotMode == Mode.Chase && (bool)PathSpline && (collision.gameObject.layer == LayerMask.NameToLayer("PlayerCollision") || collision.gameObject.layer == LayerMask.NameToLayer("Default") || collision.gameObject.layer == LayerMask.NameToLayer("Ignore Raycast")))
		{
