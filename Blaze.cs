...
public class Blaze : PlayerBase
{
	...
	public override void Awake()
	{
		...
		JumpWalkSpeed = Blaze_Lua.c_jump_walk;
		JumpTopSpeed = Blaze_Lua.c_jump_run;
	...
	private void StateJump()
	{
		...
		JumpAnimation = ((!(AirMotionVelocity.y > -3f)) ? 2 : ((Time.time - JumpTime > Blaze_Lua.c_jump_time_min || JumpBounce) ? 1 : 0));
		...
		if (JumpAttackSphere(base.transform.position, 0.5f, base.transform.forward * _Rigidbody.velocity.magnitude, 1))
		{
			...
			HalveSinkJump = false;
			JumpAnimation = 1;
			AirMotionVelocity.y = Blaze_Lua.c_jump_speed * 1.5f;
		}
	...
	private void StateFireClawStart()
	{
		...
		FireClawSpd = Blaze_Lua.c_homing_spd * ((TargetDirection != Vector3.zero) ? 1.25f : 0.75f);
		AirMotionVelocity = _Rigidbody.velocity;
		_Rigidbody.velocity = AirMotionVelocity;
	}
	...
	private void StateFireClaw()
	{
		...
		FireClawSpd = Mathf.Lerp(FireClawSpd, Blaze_Lua.c_homing_spd * ((TargetDirection != Vector3.zero) ? 1.25f : 0.5f), Time.fixedDeltaTime * 2f);
		CurSpeed = FireClawSpd;
		Vector3 vector = new Vector3(AirMotionVelocity.x, 0f, AirMotionVelocity.z);
		if (_Rigidbody.velocity.magnitude != 0f)
		{
			vector = base.transform.forward * CurSpeed;
	...
	private void StateCrowAttackStart()
	{
		...
		AttackSpeed = (IdleAttack ? TopSpeed : CurSpeed);
	...
	public override void FixedUpdate()
	{
		...
		if (Singleton<GameManager>.Instance.GameState != GameManager.State.Paused && StageManager.StageState != StageManager.State.Event)
		{
			if (!LockControls)
			{
				...
				{
					MaximumSpeed = (HasSpeedUp ? Blaze_Lua.c_speedup_speed_max : (IsGrounded() ? TopSpeed : JumpTopSpeed));
				}
			}
			else
			{
	...
	public override void Update()
	{
		...
		{
			if (CanAccelJump && Singleton<RInput>.Instance.P.GetButtonDown("Button A"))
			{
				CanAccelJump = false;
		...
		{
			if (Time.time - AccelJumpTime > Common_Lua.c_input_cd && Singleton<RInput>.Instance.P.GetButtonDown("Button X") && !FrontalCollision)
			{
				OnFireClaw();
			}
			if (Singleton<RInput>.Instance.P.GetButtonDown("Button Y"))
		...
		if (PlayerState == State.FireClaw && CanAccelJump && Time.time - FireClawTime > Common_Lua.c_input_cd && Singleton<RInput>.Instance.P.GetButtonDown("Button A"))
		{
			CanAccelJump = false;
			StateMachine.ChangeState(StateAccelJump);
		}
		...
		if (IsGrounded() && Time.time - SpinTime > Common_Lua.c_input_cd && Singleton<RInput>.Instance.P.GetButtonDown("Button A"))
		{
			StateMachine.ChangeState(StateJump);
		}
		if (!IsGrounded() && CanAccelJump && Time.time - SpinTime > Common_Lua.c_input_cd && Singleton<RInput>.Instance.P.GetButtonDown("Button A"))
		{
			CanAccelJump = false;
			StateMachine.ChangeState(StateAccelJump);
		}
