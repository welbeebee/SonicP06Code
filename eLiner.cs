	...
	private void StateChargeAttack()
	{
		if (Time.time > TimeBeforeCharge)
		{
			...
			if (TimeAfterCharge < 1.1f)
			{
				...
				if (Physics.Raycast(base.transform.position + base.transform.up * 1.25f, Mesh.forward, out var hitInfo, 0.75f, base.FrontalCol_Mask) && hitInfo.transform.GetComponent<Collider>() != GetComponent<Collider>())
				{
