...
public class Forearm : PsiObject
{
	...
	{
		StartTime = Time.time;
		base.gameObject.layer = LayerMask.NameToLayer("Stage/PhysObj_Only");
	}
	...
	{
		Exploded = true;
		Object.Instantiate(Explosion, base.transform.position - base.transform.up * ExplosionOffset, Quaternion.identity);
	...
	{
		Exploded = true;
		GameObject obj = Object.Instantiate(Explosion, base.transform.position - base.transform.up * ExplosionOffset, Quaternion.identity);
	...
	private void OnTriggerEnter(Collider collider)
	{
		...
		if (!PsychoReleased)
		{
			if (base.enabled && collider.gameObject.tag == "Vehicle")
			{
				collider.gameObject.transform.SendMessage("OnVehicleHit", 1.5f, SendMessageOptions.DontRequireReceiver);
				Explode();
			}
			if (base.enabled && (bool)collider.GetComponent<Rigidbody>())
			{
