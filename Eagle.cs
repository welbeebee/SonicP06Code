...
public class Eagle : ObjectBase
{
	...
	public void SetParameters(float _Speed1, string _Path)
	{
		Speed1 = _Speed1;
		Path = _Path;
	}
	...
	private void Start()
	{
		Spline = GameObject.Find(Path).GetComponent<BezierCurve>();
	...
	private void StateTurn()
	{
		...
		PathTime += Mathf.Lerp(0f, Speed1, num * 2f) / Spline.Length() * Time.fixedDeltaTime;
	...
	private void StateFly()
	{
		...
		float num = ((Direction.y > 0f || EagleMode == 0) ? (Speed1 * 0.75f) : Speed1);
	...
	private void StateFlyAway()
	{
		...
		{
			FlyDirection = Vector3.Slerp(FlyDirection, FlyAwayDir, Time.fixedDeltaTime * Speed1 * 0.25f).normalized;
			_Rigidbody.MovePosition(base.transform.position + FlyDirection * Speed1 * Time.fixedDeltaTime * 0.5f);
		}
	...
	private void FixedUpdate()
	{
		...
		WorldVelocity = (base.transform.position.MakePlanarRaw() - LastPosition) / Time.fixedDeltaTime;
		LastPosition = base.transform.position.MakePlanarRaw();
	}
	...
	private void Update()
	{
		...
		Animator.SetFloat("Planar Speed", WorldVelocity.magnitude);
		if ((bool)PM && Singleton<GameManager>.Instance.GameState != GameManager.State.Paused && PM.Base.GetState() == "Hold" && !LockPlayer && Singleton<RInput>.Instance.P.GetButtonDown("Button A"))
		{
