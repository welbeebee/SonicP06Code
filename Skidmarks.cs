...
public class Skidmarks : Singleton<Skidmarks>
{
	...
	private const float MIN_SQR_DISTANCE = 0.25f;
	...
	protected void Start()
	{
		...
		mr.material = Resources.Load("DefaultPrefabs/Objects/Common/Vehicle/Prefab/SkidmarksMat", typeof(Material)) as Material;
	...
	public int AddSkidMark(Vector3 pos, Vector3 normal, float opacity, float width, int lastIndex)
	{
		if (opacity > 1f)
		{
			opacity = 1f;
		...
		return AddSkidMark(pos, normal, black, width, lastIndex);
	}

	public int AddSkidMark(Vector3 pos, Vector3 normal, Color32 colour, float width, int lastIndex)
	{
		if (colour.a == 0)
		{
			return -1;
		...
		if (lastIndex != -1)
		{
			...
			if (lhs.sqrMagnitude < 0.25f)
			{
				return lastIndex;
			}
			if (lhs.sqrMagnitude > 2.5f)
			{
				lastIndex = -1;
				markSection = null;
			}
		...
		if (markSection != null)
		{
			...
			if (markSection.LastIndex == -1)
			{
				...
				markSection.Posl = markSection2.Pos + normalized * width * 0.5f;
				markSection.Posr = markSection2.Pos - normalized * width * 0.5f;
			}
		}
		UpdateSkidmarksMesh();
