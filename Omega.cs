...
public class Omega : PlayerBase
{
	...
	public override void Awake()
	{
		...
		JumpWalkSpeed = Omega_Lua.c_jump_walk;
		JumpTopSpeed = Omega_Lua.c_jump_run;
	...
	private void StateOmegaShotStart()
	{
		...
		ComboShotCount++;
		ComboShotSpeed = ((ComboShotCount < 3) ? (TopSpeed * ((TargetDirection != Vector3.zero) ? 1.05f : 0.65f)) : 0f);
	...
	private void StateOmegaShot()
	{
		...
		if (ComboShotCount < 3)
		{
			ComboShotSpeed = Mathf.MoveTowards(ComboShotSpeed, 0f, Time.fixedDeltaTime * 30f);
		...
		else
		{
			...
			if (StartShotFlurry && Time.time > FlurryTime + (float)ShotCount * 0.1325f && ShotCount < MaxShots)
			{
				...
				AttackSphere(OmegaEffects.ShotBones[ShotArmID].position + OmegaEffects.ShotBones[ShotArmID].forward * 1.5f, Omega_Lua.c_omega_shot.radius, OmegaEffects.ShotBones[ShotArmID].forward * Omega_Lua.c_omega_shot_power, Omega_Lua.c_omega_shot_damage);
	...
	public override void FixedUpdate()
	{
		base.FixedUpdate();
		UpdateMesh();
		if ((PlayerState == State.Ground || PlayerState == State.Jump || PlayerState == State.Air || PlayerState == State.SlowFall || PlayerState == State.Hover || (PlayerState == State.Spring && !LockControls) || (PlayerState == State.WideSpring && !LockControls) || (PlayerState == State.JumpPanel && !LockControls) || (PlayerState == State.RainbowRing && !LockControls)) && !IsLockOnShot)
		{
			...
			if (!LockControls)
			{
				...
				if (PlayerState != State.OmegaLauncher)
				{
					if (PlayerState != State.Hover)
					{
						...
						{
							MaximumSpeed = (HasSpeedUp ? Omega_Lua.c_speedup_speed_max : (IsGrounded() ? TopSpeed : JumpTopSpeed));
						}
					}
					else
					{
						MaximumSpeed = JumpTopSpeed * 1.3f;
					}
				}
				else
				{
					MaximumSpeed = JumpTopSpeed * 0.25f;
				}
			}
			else
			{
	...
	public override void Update()
	{
		...
		if (PlayerState == State.OmegaShot)
		{
			...
			if (Time.time - ComboShotTimer > Common_Lua.c_input_cd && Singleton<RInput>.Instance.P.GetButtonDown("Button A"))
			{
				StateMachine.ChangeState(StateJump);
			}
		}
		...
		if (Time.time - GunTime > Common_Lua.c_input_cd && Singleton<RInput>.Instance.P.GetButtonDown("Button A"))
		{
			StateMachine.ChangeState(StateJump);
			if (GunState == 1)
			{
