...
public class Glider : VehicleBase
{
	...
	public override void Update()
	{
		...
		if ((bool)PM)
		{
			...
			PM.Base.CamOffset.y = Mathf.Lerp(PM.Base.CamOffset.y, Common_Lua.c_camera.y + 0.375f + ((VehicleState == 1 && !VehicleStart) ? ((0f - InputY()) * 2f) : 0f), num2);
		}
	...
	private void UpdateMesh()
	{
		...
		{
			Meshes[i].enabled = ImmunityTime - Time.time <= 0f || ((ImmunityTime - Time.time >= 0f && BlinkTimer <= 0.5f) ? true : false);
		}
	}
	...
	{
		if (IsMounted && !VehicleStart && !VehicleDamage && (collision.gameObject.layer == LayerMask.NameToLayer("Default") || collision.gameObject.layer == LayerMask.NameToLayer("DefaultObject") || collision.gameObject.layer == LayerMask.NameToLayer("PlayerCollision") || collision.gameObject.layer == LayerMask.NameToLayer("PlayerCollisionObject") || (collision.gameObject.layer == LayerMask.NameToLayer("BreakableObj") && !UseBoost) || (bool)collision.gameObject.GetComponent<HurtVehicle>()))
		{
	...
	{
		ReflVel = -base.transform.forward * 10f;
		RawSpeed = 0f;
	}
	...
	private void OnCrash()
	{
		...
		ImmunityTime = Time.time + DamageAmount;
	...
	public override void OnDamage(float Damage, bool KnockBack = false)
	{
		if (!VehicleDamage)
		{
