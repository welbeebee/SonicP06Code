...
public class Aqa_Magnet : MonoBehaviour
{
	...
	private void OnHit(HitInfo HitInfo)
	{
		...
		{
			if (Kind != Type.Switch || RepelMode || !(Time.time - StartTime > 1f))
			{
				return;
			}
			...
			Collider[] array2 = Physics.OverlapSphere(AttractPoint.position, Radius);
			if (array2 != null)
			{
				for (int j = 0; j < array2.Length; j++)
				{
					if ((bool)array2[j].GetComponent<EnemyBase>())
					{
						array2[j].gameObject.SendMessage("OnCommandDestroy", new HitInfo(HitInfo.player, Vector3.zero, 10), SendMessageOptions.DontRequireReceiver);
					}
					array2[j].SendMessage("OnDeflect", base.transform, SendMessageOptions.DontRequireReceiver);
				}
			}
			StartTime = Time.time;
			RepelStartFX.Play();
