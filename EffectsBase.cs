...
public class EffectsBase : MonoBehaviour
{
	...
	internal GrindFX RailParticles;
	...
	{
		if (JumpFX != null && (Singleton<Settings>.Instance.settings.SpinEffect != 3 || (Singleton<Settings>.Instance.settings.SpinEffect == 3 && PM.Base.GetPrefab("tails"))))
		{
			UpdateJumpFX();
		}
	...
	private void FixedUpdate()
	{
		if (UseJumpFX && Singleton<Settings>.Instance.settings.SpinEffect == 2)
		{
			float num = (PM.Base.GetPrefab("sonic_fast") ? 1.5f : 1f);
			JumpSA2Pos[0] = Vector3.Lerp(JumpSA2Pos[0], base.transform.position + base.transform.up * 0.25f, Time.fixedDeltaTime * 25f * num);
			JumpSA2Pos[1] = Vector3.Lerp(JumpSA2Pos[1], base.transform.position + base.transform.up * 0.25f, Time.fixedDeltaTime * 19f * num);
			JumpSA2Pos[2] = Vector3.Lerp(JumpSA2Pos[2], base.transform.position + base.transform.up * 0.25f, Time.fixedDeltaTime * 15f * num);
	...
	public void UpdateJumpBallFX(bool Conditions, Renderer Mesh = null)
	{
		...
		if (Conditions)
		{
			JumpballBlink += Time.deltaTime * 30f;
			JumpballTime += Time.deltaTime * 2f;
			if (JumpballBlink >= 1f)
			{
				JumpballBlink = 0f;
			}
			...
			if ((bool)Mesh)
			{
				Mesh.enabled = JumpballTime > 0.5f || (JumpballBlink <= 0.25f && JumpballTime < 0.5f);
			}
			else
			{
				JumpballRenderer.enabled = JumpballTime > 0.5f || (JumpballBlink <= 0.25f && JumpballTime < 0.5f);
			}
		}
		else if ((bool)Mesh)
		{
	...
	{
		DestroyRailFX();
		RailParticles = Object.Instantiate(GrindFX[RailType], base.transform.position + base.transform.up * -0.25f, base.transform.rotation).GetComponent<GrindFX>();
		RailParticles.transform.SetParent(base.transform);
		RailParticles.Player = PM.Base;
