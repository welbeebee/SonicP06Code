...
public class EnemyBase : PsiObject
{
	...
	private LayerMask EventBox_Mask => LayerMask.GetMask("Default", "DefaultObject");

	internal LayerMask Collision_Mask => LayerMask.GetMask("PlayerCollision", "PlayerCollisionObject");

	internal LayerMask FrontalCol_Mask => LayerMask.GetMask("Enemy");

	public void SetParameters(bool _Restart, bool _FindPlayer, bool _IsFixed, string _AppearPath, float _AppearVelocity, string _ActionPath, Vector3 _HomingTarget)
	{
		Restart = _Restart;
	...
	public override void Awake()
	{
		...
		{
			EnemyRespawn enemyRespawn = new GameObject(base.gameObject.name + "_respawner").AddComponent<EnemyRespawn>();
			enemyRespawn.Enemy = this;
			enemyRespawn.Enemy.Restart = false;
			enemyRespawn.Enemy.IsRespawn = true;
			enemyRespawn.ObjectManager = GameObject.Find("Enemies");
			enemyRespawn.StartPos = base.transform.position;
			enemyRespawn.StartRot = base.transform.rotation;
			enemyRespawn.transform.SetParent(null);
			IsRespawn = true;
			SetupRestart = true;
		}
	}
	...
	internal Quaternion FaceTarget(Vector3 TargetPos, Quaternion InRotation, bool Inverse = false, float Speed = 0.75f, bool Smooth = false)
	{
	...
	public virtual void OnCollisionEnter(Collision collision)
	{
		if (PsychoThrown)
		{
			collision.gameObject.SendMessage("OnHit", new HitInfo(Target.transform, _Rigidbody.velocity, 10), SendMessageOptions.DontRequireReceiver);
			AutoDestroy();
		}
	}
	...
	private void ManagerHurtScrpts(bool Block)
	{
		for (int i = 0; i < HurtScripts.Length; i++)
		{
			HurtScripts[i].BlockDmg = Block;
		}
	}
	...
	private void OnPsychoThrow(HitInfo HitInfo)
	{
		...
		_Rigidbody.velocity = HitInfo.force;
	}
