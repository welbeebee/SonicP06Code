...
public class Amy : PlayerBase
{
	...
	public override void Awake()
	{
		...
		JumpWalkSpeed = Amy_Lua.c_jump_walk;
		JumpTopSpeed = Amy_Lua.c_jump_run;
	...
	private void StateJump()
	{
		...
		JumpAnimation = ((!(AirMotionVelocity.y > -3f)) ? 2 : ((Time.time - JumpTime > Amy_Lua.c_jump_time_min || JumpBounce) ? 1 : 0));
		if (!Singleton<RInput>.Instance.P.GetButton("Button A"))
		{
			ReleasedKey = true;
		}
		if (Singleton<GameManager>.Instance.GameState != GameManager.State.Paused && !ReleasedKey && Singleton<RInput>.Instance.P.GetButton("Button A") && Time.time - JumpTime < (JumpBounce ? 0.25f : ((!IsDoubleJump && !HalveSinkJump) ? 0.7f : 0.45f)))
		{
		...
		if (JumpAttackSphere(base.transform.position, 0.5f, base.transform.forward * _Rigidbody.velocity.magnitude, 1))
		{
			...
			HalveSinkJump = false;
			JumpAnimation = 1;
			AirMotionVelocity.y = Amy_Lua.c_jump_speed * 1.5f;
		}
	...
	private void StateHammerAttackStart()
	{
		...
		HammerSpeed = Mathf.Max(WalkSpeed * 2f, CurSpeed);
		AttackDamage = true;
		HammerSmash = false;
		...
		StandHammerAttack = CurSpeed < WalkSpeed * 3f;
		PlayAnimation("Movement (Blend Tree)", "On Ground");
	}
	...
	private void StateHammerSpinStart()
	{
		...
		HammerSpeed = Mathf.Min(CurSpeed, WalkSpeed * 6f);
		HammerSpinAnim = 0f;
	...
	private void StateHammerSpin()
	{
		...
		{
			if (Time.time - HammerSpinTimer > 4.75f)
			{
			...
			if (Time.time - HammerSpinTimer > 6f)
			{
				StateMachine.ChangeState(StateGround);
			}
		}
	...
	public override void FixedUpdate()
	{
		...
		if (Singleton<GameManager>.Instance.GameState != GameManager.State.Paused && StageManager.StageState != StageManager.State.Event)
		{
			if (!LockControls)
			{
				...
				{
					MaximumSpeed = ((!HasSpeedUp) ? (TopSpeed * 1.25f) : Amy_Lua.c_speedup_speed_max);
				}
				else if (PlayerState == State.HammerSpin && HammerSpinState > 0)
				{
					MaximumSpeed = WalkSpeed * ((HammerSpinState == 1) ? 8f : 1f);
				}
				else if (WalkSwitch)
				{
				...
				{
					MaximumSpeed = (HasSpeedUp ? Amy_Lua.c_speedup_speed_max : (IsGrounded() ? TopSpeed : JumpTopSpeed));
				}
			}
			else
			{
	...
	public override void Update()
	{
		...
		if (((PlayerState == State.Ground || PlayerState == State.Path || PlayerState == State.DashPanel || PlayerState == State.Balancer) && CurSpeed > 21f) || PlayerState == State.HammerAttack || (PlayerState == State.HammerSpin && HammerSpinState < 2) || PlayerState == State.HammerJump || PlayerState == State.AirHammerSpin)
		{
			if (!SwitchHammer)
			{
		...
		if (Singleton<GameManager>.Instance.GameState != GameManager.State.Paused && Singleton<GameManager>.Instance.GameState != GameManager.State.Result && StageManager.StageState != StageManager.State.Event && !IsDead && PlayerState != State.Talk)
		{
			...
			if (PlayerState == State.HammerSpin)
			{
				...
				if (Time.time - HammerSpinTimer > Common_Lua.c_input_cd && HammerSpinState < 2 && Singleton<RInput>.Instance.P.GetButtonDown("Button A"))
				{
	...
	internal void OnJump(bool DoubleJump = false)
	{
