...
public class PathSnowBall : MonoBehaviour
{
	...
	private void Update()
	{
		...
		Progress += CurSpeed / Curve.Length() * Time.deltaTime;
		if (Progress > 1f)
		{
			if (FX.activeSelf)
			{
		...
		CurSpeed = Mathf.Lerp(CurSpeed, Speed, Time.deltaTime * 0.75f);
		...
		Model.transform.Rotate(CurSpeed * Time.deltaTime * num, 0f, 0f);
		Model.transform.localScale = new Vector3(num2, num2, num2);
		Point.transform.localPosition = localPosition;
		if (TrappedPlayer)
		{
			Point.transform.Rotate(CurSpeed * Time.deltaTime * num, 0f, 0f);
		}
		...
		num3 = ((num3 > 0f) ? 1f : (-1f));
		if (TrappedPlayer || num3 != 1f || (HitTime != -1f && !(Time.time - HitTime > 0.5f)))
		{
			return;
		}
		...
		if (component.HUD.Rings > 0)
		{
			CurSpeed = 0f;
			component.OnHurtEnter(3);
			component.transform.forward = base.transform.forward;
			if (component.GetPrefab("snow_board"))
			{
				component.PlayerManager.snow_board.Speed = base.transform.forward * Speed;
			}
			else
			{
				component.CurSpeed = Speed;
			}
		}
		else
		{
