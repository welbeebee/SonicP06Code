...
public class Silver : PlayerBase
{
	...
	private bool UpheaveSmashCanUseMoves;
	...
	public override void Awake()
	{
		...
		JumpWalkSpeed = Silver_Lua.c_jump_walk;
		JumpTopSpeed = Silver_Lua.c_jump_run;
	...
	private void StateGround()
	{
		...
		if (Vector3.Dot(TargetDirection, _Rigidbody.velocity.normalized) < -0.75f && CurSpeed > WalkSpeed * 1.5f)
		{
		...
		if (!IsGrounded() || (CurSpeed <= WalkSpeed && ShouldAlignOrFall(Align: true)))
		{
			StateMachine.ChangeState(StateAir);
		}
	}
	...
	private void StateBrakeStart()
	{
		...
		BrakeSpd = CurSpeed;
		BrakeDecelSpeed = BrakeSpeed - Mathf.Min(CurSpeed, TopSpeed) / TopSpeed * 20f;
		if (BrakeSpd > TopSpeed)
		{
			BrakeDecelSpeed += BrakeSpd - TopSpeed;
		}
		BrakeStopped = false;
	}
	...
	private void StateJump()
	{
		...
		if (HasLotusOfResilience)
		{
			...
			JumpAnimation = ((!(AirMotionVelocity.y > -3f)) ? 2 : ((Time.time - JumpTime > Silver_Lua.c_jump_time_min || JumpBounce) ? 1 : 0));
			if (JumpAttackSphere(base.transform.position, 0.5f, base.transform.forward * _Rigidbody.velocity.magnitude, 1))
			{
				...
				HalveSinkJump = false;
				JumpAnimation = 1;
				AirMotionVelocity.y = Silver_Lua.c_jump_speed * 1.5f;
			}
		}
		...
		AirMotionVelocity.y = LimitVel(AirMotionVelocity.y, (!JumpBounce) ? 14f : AirMotionVelocity.y);
		if (!Singleton<RInput>.Instance.P.GetButton("Button A"))
		{
			ReleasedKey = true;
		}
		if (Singleton<GameManager>.Instance.GameState != GameManager.State.Paused && !ReleasedKey && Singleton<RInput>.Instance.P.GetButton("Button A") && Time.time - JumpTime < (JumpBounce ? 0.25f : ((!HalveSinkJump) ? 0.7f : 0.45f)))
		{
			AirMotionVelocity += Vector3.up * ((!HalveSinkJump) ? 4.25f : 3f) * Time.fixedDeltaTime * ((!JumpBounce) ? 4f : 6.75f);
		}
	...
	private void StateLevitate()
	{
		...
		AirMotionVelocity.y = Mathf.Lerp(AirMotionVelocity.y, -0.001f, Time.fixedDeltaTime * 9f);
		GeneralMeshRotation = Quaternion.LookRotation(ForwardMeshRotation, UpMeshRotation);
		base.transform.rotation = Quaternion.FromToRotation(base.transform.up, Vector3.up) * base.transform.rotation;
		_Rigidbody.velocity = AirMotionVelocity;
		DoWallNormal();
	...
	private void StateUpheaveSmash()
	{
		...
		AirMotionVelocity.y -= 25f * Time.fixedDeltaTime;
		AirMotionVelocity.y = LimitVel(AirMotionVelocity.y, 14f);
		...
		if (Time.time - UpheaveSmashTime > 0.15f && !UpheaveSmashCanUseMoves)
		{
			UpheaveSmashCanUseMoves = true;
		}
		else if (!UpheaveSmashCanUseMoves)
		{
			AirActionTime = Time.time;
		}
		MaxRayLenght = ((AirMotionVelocity.y < 0f) ? 0.75f : 0f);
		GeneralMeshRotation = Quaternion.LookRotation(ForwardMeshRotation, UpMeshRotation);
		base.transform.rotation = Quaternion.FromToRotation(base.transform.up, Vector3.up) * base.transform.rotation;
		_Rigidbody.velocity = AirMotionVelocity;
		DoWallNormal();
		if (IsGrounded() && ShouldAlignOrFall(Align: false) && AirMotionVelocity.y < 0f)
		{
			StateMachine.ChangeState(StateGround);
			DoLandAnim();
			PlayerManager.PlayerEvents.CreateLandFXAndSound();
	...
	private void StatePsychoSmash()
	{
		...
		else
		{
			...
			{
				CurSpeed = 0f;
				AirMotionVelocity.y = Mathf.Lerp(AirMotionVelocity.y, 0f, Time.fixedDeltaTime * 9f);
			}
			if (IsGrounded())
			{
				StateMachine.ChangeState(StateGround);
	...
	private void StateDash()
	{
		...
		if ((Time.time - DashTime > Silver_Lua.c_tele_dash_time && ((HasLotusOfResilience && !Singleton<RInput>.Instance.P.GetButton("Button A")) || (!HasLotusOfResilience && ((Singleton<RInput>.Instance.P.GetButton("Button A") && Time.time - DashTime > Silver_Lua.c_tele_dash_time + 0.25f) || !Singleton<RInput>.Instance.P.GetButton("Button A"))))) || HUD.ActionDisplay < 1f || FrontalCollision)
		{
			if (FrontalCollision)
			{
				Dtd_Billiard component = FrontalHit.collider.GetComponent<Dtd_Billiard>();
				if ((bool)component)
				{
					component.OnPush();
				}
	...
	private void StatePsychoShock()
	{
		...
		else
		{
			...
			if (Time.time - ShockTime > 0.15f && !ShockRelease)
			{
				...
				DoPsychoShock(base.transform.position - base.transform.up * 0.1f, Silver_Lua.c_radius);
				ShockRelease = true;
			}
	...
	private void StateESPAwaken()
	{
		...
		if (Time.time - AwakenTimer > 0.4f && !HasAwakened)
		{
			...
			HUD.LevelAnimator.SetBool("Is Highlight", value: true);
	...
	public override void FixedUpdate()
	{
		...
		if (Singleton<GameManager>.Instance.GameState != GameManager.State.Paused && StageManager.StageState != StageManager.State.Event)
		{
			if (!LockControls)
			{
				RotatePlayer(Silver_Lua.c_rotation_speed, Override: false, PlayerState == State.Levitate);
				AccelerationSystem(((!HasSpeedUp) ? Silver_Lua.c_run_acc : Silver_Lua.c_speedup_acc) * ((PlayerState != State.Levitate) ? 1f : 2.5f));
				if (PlayerState != State.Levitate && WalkSwitch)
				{
					MaximumSpeed = (IsGrounded() ? WalkSpeed : JumpWalkSpeed);
				}
				else
				{
					MaximumSpeed = ((PlayerState == State.Levitate) ? TopSpeed : (HasSpeedUp ? Silver_Lua.c_speedup_speed_max : (IsGrounded() ? TopSpeed : JumpTopSpeed)));
				}
			}
			else
			{
	...
	public override void Update()
	{
		...
		if (PlayerState == State.UpReel || PlayerState == State.Balancer)
		{
			AirActionTime = Time.time;
		}
		...
		{
			bool flag = PlayerState == State.Ground && HasSigilOfAwakening && !IsAwakened && HUD.ESPMaturityDisplay >= 30f && Singleton<RInput>.Instance.P.GetAxis("D-Pad Y") > 0f;
			...
			UsingPsychokinesis = !LockControls && !IsDead && Singleton<RInput>.Instance.P.GetButton("Right Trigger") && ManipulateObjects && !OnUpheave() && !flag && PlayerState != State.Levitate && PlayerState != State.Upheave && PlayerState != State.Hotspot && PlayerState != State.Upheave && PlayerState != State.UpheaveSmash;
			...
			if (PlayerState == State.UpheaveSmash && UpheaveSmashCanUseMoves)
			{
			...
			if (PlayerState == State.PsychoSmash && PSmashState == 1)
			{
				...
				if (Time.time - PSmashTime > 0.3f && (TargetDirection != Vector3.zero || Singleton<RInput>.Instance.P.GetButton("Right Trigger")))
				{
					...
					{
						CurSpeed = Mathf.Max(WalkSpeed, CurSpeed);
					}
					if (Singleton<RInput>.Instance.P.GetButton("Right Trigger"))
					{
						ManipulateObjects = true;
			...
			if (IsAwakened)
			{
				...
				{
					HUD.LevelAnimator.SetBool("Is Highlight", value: false);
					IsAwakened = false;
				}
			}
		}
	...
	public override void SetStoredAttributes()
	{
		...
		for (int i = 0; i < Singleton<GameManager>.Instance.StoredPlayerVars.Length; i++)
		{
			if (Singleton<GameManager>.Instance.StoredPlayerVars[i].VarName == "ESPGeneral")
			{
				...
				if (Singleton<GameManager>.Instance.StoredPlayerVars[i].VarBool)
				{
					IsAwakened = true;
					HUD.LevelAnimator.SetBool("Is Highlight", value: true);
				}
	...
	private void DoPsychokinesis(string RadiusType = "Psychokinesis")
	{
		...
		{
			if (!PickedObjects[j].GetComponent<Parasol>() && !PickedObjects[j].GetComponent<Missile>() && !PickedObjects[j].GetComponent<DarkShot>() && !PickedObjects[j].GetComponent<TimedBomb>() && !PickedObjects[j].GetComponent<Forearm>() && !PickedObjects[j].GetComponent<EnemyBase>())
			{
	...
	private void DoPsychoThrow(Vector3 Position, float Radius, bool CapsuleRadius = false)
	{
		if (PickedObjects.Count == 0)
		{
			Collider[] array = (CapsuleRadius ? Physics.OverlapCapsule(Position, Position + base.transform.forward * 1.5f, Radius) : Physics.OverlapSphere(Position, Radius));
			for (int i = 0; i < array.Length; i++)
			{
				...
				if (ThrowForce == Silver_Lua.c_psychosmash_power && ((bool)array[i].GetComponent<BreakWall>() || ((bool)array[i].GetComponent<PhysicsObj>() && (bool)array[i].GetComponent<PhysicsObj>().brokenPrefab && !array[i].GetComponentInChildren<Rigidbody>())))
				{
					array[i].SendMessage("OnHit", new HitInfo(base.transform, base.transform.forward * ThrowForce, 10), SendMessageOptions.DontRequireReceiver);
				}
		...
		else if (ThrowForce == Silver_Lua.c_psychosmash_power)
		{
			...
			for (int l = 0; l < PickedObjects.Count; l++)
			{
				...
				if (!PickedObjects[l].GetComponent<Parasol>() && !PickedObjects[l].GetComponent<Missile>() && !PickedObjects[l].GetComponent<DarkShot>() && !PickedObjects[l].GetComponent<TimedBomb>() && !PickedObjects[l].GetComponent<Forearm>() && !PickedObjects[l].GetComponent<EnemyBase>())
				{
					PickedObjects[l].SendMessage("PsychoThrowSetup", value, SendMessageOptions.DontRequireReceiver);
				}
				int damage2 = ((!IsAwakened) ? (HasFlameOfControl ? 5 : 2) : 10);
				PickedObjects[l].SendMessage("OnPsychoThrow", new HitInfo(base.transform, vector * Common_Lua.c_psi_throw_speed, damage2), SendMessageOptions.DontRequireReceiver);
		...
		else
		{
			...
			if (!PickedObjects[0].GetComponent<Parasol>() && !PickedObjects[0].GetComponent<Missile>() && !PickedObjects[0].GetComponent<DarkShot>() && !PickedObjects[0].GetComponent<TimedBomb>() && !PickedObjects[0].GetComponent<Forearm>() && !PickedObjects[0].GetComponent<EnemyBase>())
			{
				PickedObjects[0].SendMessage("PsychoThrowSetup", new List<GameObject>(PickedObjects), SendMessageOptions.DontRequireReceiver);
			}
			int damage3 = ((!IsAwakened) ? 1 : 10);
			PickedObjects[0].SendMessage("OnPsychoThrow", new HitInfo(base.transform, vector2 * Common_Lua.c_psi_throw_speed, damage3), SendMessageOptions.DontRequireReceiver);
	...
	{
		if (HUD.ActionDisplay > 33.333332f && !UsingPsychokinesis && Physics.Raycast(base.transform.position + base.transform.up * 0.25f, -base.transform.up, out var hitInfo, 0.6f))
		{
			PhysicsObj component = hitInfo.transform.GetComponent<PhysicsObj>();
			if ((bool)hitInfo.transform.GetComponent<Rigidbody>() && hitInfo.transform.gameObject.layer == LayerMask.NameToLayer("BreakableObj") && !hitInfo.transform.GetComponent<Parasol>() && (!component || ((bool)component && (bool)component.Renderer)))
			{
	...
	private void UpdateMesh()
	{
		...
		{
			BlinkTimer += Time.fixedDeltaTime * Common_Lua.c_blink_spd;
			if (BlinkTimer > 1f)
			{
				BlinkTimer = 0f;
			}
