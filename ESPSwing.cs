...
public class ESPSwing : PsiObject
{
	...
	private void Start()
	{
		...
		CameraParams = new CameraParameters(32, Vector3.zero, Vector3.zero, CamPos, CamTgt);
		Progress = 0f;
		RotFactor = 0f;
	...
	private void FixedUpdate()
	{
		...
		else
		{
			...
			if (RotFactor > 0.1f && HitTimer > 0.5f && (AttackSphere(OriginPoint.position + OriginPoint.forward * 7.5f, OriginPoint.forward * 30f + OriginPoint.up * 2f) || AttackSphere(OriginPoint.position + OriginPoint.forward * -7.5f, OriginPoint.forward * -30f + OriginPoint.up * 2f)))
			{
				HitTimer = 0f;
				Audio[1].Play();
			}
	...
	private void OnEventSignal()
	{
		if (!IsLifting && (bool)Player)
		{
			if (Vector3.Dot(base.transform.forward, Player.transform.forward) < 0f && !IsReverse)
			{
				...
				CameraParams = new CameraParameters(32, Vector3.zero, Vector3.zero, CamPosReverse, CamTgtReverse);
				IsReverse = true;
			}
