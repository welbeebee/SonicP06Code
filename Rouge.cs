...
public class Rouge : PlayerBase
{
	...
	public override void Awake()
	{
		...
		JumpWalkSpeed = Rouge_Lua.c_jump_walk;
		JumpTopSpeed = Rouge_Lua.c_jump_run;
	...
	private void StateJump()
	{
		...
		JumpAnimation = ((!(AirMotionVelocity.y > -3f)) ? 2 : ((Time.time - JumpTime > Rouge_Lua.c_jump_time_min || JumpBounce) ? 1 : 0));
		...
		if (JumpAttackSphere(base.transform.position, 0.5f, base.transform.forward * _Rigidbody.velocity.magnitude, 1))
		{
			...
			HalveSinkJump = false;
			JumpAnimation = 1;
			AirMotionVelocity.y = Rouge_Lua.c_jump_speed * 1.5f;
		}
	...
	private void StateGlideStart()
	{
		...
		GlideSources[0].Play();
		GlideSources[1].Play();
		GlideSpeed = Mathf.Max(Rouge_Lua.c_flight_acc, CurSpeed);
		GlideYGrav = 0f;
		AirMotionVelocity = _Rigidbody.velocity;
	...
	private void StateGlide()
	{
		...
		if (TargetDirection.magnitude != 0f)
		{
			GlideSpeed += Rouge_Lua.c_flight_acc * Time.fixedDeltaTime * ((GlideSpeed > Rouge_Lua.c_flight_speed_min) ? 1f : 2f);
		}
		...
		{
			GlideSpeed += Rouge_Lua.c_flight_acc * Time.fixedDeltaTime * 2f;
		}
	...
	private void StateClimb()
	{
		...
		LeftStickX = ((!SetBomb) ? Mathf.MoveTowards(LeftStickX, Singleton<RInput>.Instance.P.GetAxis("Left Stick X") * Rouge_Lua.c_climb_speed, Time.fixedDeltaTime * 50f) : 0f);
		LeftStickY = ((!SetBomb) ? Mathf.MoveTowards(LeftStickY, Singleton<RInput>.Instance.P.GetAxis("Left Stick Y") * Rouge_Lua.c_climb_speed, Time.fixedDeltaTime * 50f) : 0f);
	...
	private void StateKickStart()
	{
		...
		KickTimer = Time.time;
		KickCount++;
		KickSpeed = TopSpeed * ((TargetDirection != Vector3.zero) ? 1f : 0.5f);
		KickDamage = true;
		QueuedPress = false;
	...
	private void StateBomb()
	{
		...
		else if (CurSpeed > WalkSpeed)
		{
			CurSpeed -= WalkSpeed * Time.fixedDeltaTime * 5f;
		}
		if (!BombGrounded)
		{
			Vector3 vector = new Vector3(AirMotionVelocity.x, 0f, AirMotionVelocity.z);
	...
	public override void FixedUpdate()
	{
		...
		if (Singleton<GameManager>.Instance.GameState != GameManager.State.Paused && StageManager.StageState != StageManager.State.Event)
		{
			if (!LockControls)
			{
				...
				{
					MaximumSpeed = (HasSpeedUp ? Rouge_Lua.c_speedup_speed_max : (IsGrounded() ? TopSpeed : JumpTopSpeed));
				}
			}
			else
			{
	...
	public override void Update()
	{
		...
		if (Singleton<GameManager>.Instance.GameState != GameManager.State.Paused && Singleton<GameManager>.Instance.GameState != GameManager.State.Result && StageManager.StageState != StageManager.State.Event && !IsDead && PlayerState != State.Talk)
		{
			...
			if (PlayerState == State.Jump || PlayerState == State.Air || PlayerState == State.SlowFall || PlayerState == State.BallFall || (PlayerState == State.ClimbUp && Time.time - ClimbUpTime > 0.15f) || (PlayerState == State.FlyingKick && Time.time - FlyingKickTime > 0.55f) || (PlayerState == State.KickDive && KickDiveState == 0) || (PlayerState == State.WideSpring && !LockControls) || (PlayerState == State.RainbowRing && !LockControls) || (PlayerState == State.Lotus && !LockControls) || (PlayerState == State.Tarzan && !LockControls))
			{
				if (!LockControls && !GlideReleasedKey && !Singleton<RInput>.Instance.P.GetButton("Button A"))
				{
					GlideReleasedKey = true;
				}
				if (((PlayerState == State.Jump && ReleasedKey) || (PlayerState != State.Jump && GlideReleasedKey)) && Singleton<RInput>.Instance.P.GetButton("Button A") && ((CanGlide && Time.time - GlideKeyTime > Common_Lua.c_input_cd) || (!CanGlide && Time.time - GlideLockTime > Rouge_Lua.c_flight_cd)))
				{
					CanGlide = false;
			...
			if (PlayerState == State.Kick)
			{
				...
				if (Time.time - KickTimer > Common_Lua.c_input_cd && Singleton<RInput>.Instance.P.GetButtonDown("Button A"))
				{
					StateMachine.ChangeState(StateJump);
				}
			}
