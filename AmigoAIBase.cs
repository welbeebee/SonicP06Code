...
public class AmigoAIBase : MonoBehaviour
{
	...
	[Header("Amy")]
	public Renderer AmyHammerRenderer;

	public ParticleSystem[] AmyTarotFX;

	public ParticleSystem[] AmyHammerInvokeFX;

	public AudioSource AmyTarotAudioSource;

	private bool AmySwitchHammer;
	...
	internal LayerMask FrontalCol_Mask => LayerMask.GetMask("Default", "DefaultObject", "PlayerCollision", "PlayerCollisionObject", "BreakableObj", "Object/PlayerOnlyCol", "Vehicle");
	...
	{
		if (Params.JumpFX != null && (Singleton<Settings>.Instance.settings.SpinEffect != 3 || (Singleton<Settings>.Instance.settings.SpinEffect == 3 && Params.AmigoName == "tails")))
		{
			Params.UpdateJumpFX();
		}
		...
		if (Params.AmigoName == "silver")
		{
			...
			for (int n = 0; n < SilverRenderers.Length; n++)
			{
				...
				PropBlock.SetFloat("_GlowInt", Mathf.Lerp(PropBlock.GetFloat("_GlowInt"), 0f, Time.deltaTime * 10f));
		...
		if (Params.AmigoName == "amy")
		{
			if ((GetAmigoState() == "Ground" || GetAmigoState() == "Path" || GetAmigoState() == "DashPanel") && CurSpeed > 21f)
			{
				if (!AmySwitchHammer)
				{
					PlayAmyHammerInvokeFX();
					AmySwitchHammer = true;
				}
				AmyHammerRenderer.enabled = true;
			}
			else if (AmySwitchHammer)
			{
				AmyHammerRenderer.enabled = false;
				PlayAmyHammerInvokeFX();
				AmySwitchHammer = false;
			}
		}
	...
	public void PlayAmyTarotFX(bool Enable)
	{
		for (int i = 0; i < AmyTarotFX.Length; i++)
		{
			if (Enable)
			{
				AmyTarotFX[i].gameObject.SetActive(value: true);
				AmyTarotFX[i].Play();
				AmyTarotAudioSource.Play();
			}
			else
			{
				AmyTarotFX[i].gameObject.SetActive(value: false);
				AmyTarotAudioSource.Stop();
			}
		}
	}

	public void PlayAmyHammerInvokeFX()
	{
		for (int i = 0; i < AmyHammerInvokeFX.Length; i++)
		{
			AmyHammerInvokeFX[i].Play();
		}
	}
	...
	public virtual void OnSpring(Vector3 Pos, Vector3 Vel, float Timer, string LaunchAnimMode, bool UseTimerToExit, bool UseTimerToRelease, bool AlwaysLocked, bool LandingStop)
	{
	}
	...
	public virtual void OnJumpPanel(Vector3 Pos, Vector3 Vel, float Timer, string LaunchAnimMode, bool UseTimerToExit, bool UseTimerToRelease, bool AlwaysLocked, bool LandingStop, bool StopOnTimer, float StopTimer)
	{
	}
	...
	{
		if ((!Force && CurSpeed <= 0f && _Rigidbody.velocity.y < -10f && Params.AmigoName != "sonic_fast") || Force)
		{
