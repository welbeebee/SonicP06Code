...
public class AmigoAI : AmigoAIBase
{
	...
	private bool AmyStopTarotEffect;
	...
	private bool SpringLandingStop;
	...
	private bool JumpPanelLandingStop;

	private string JumpPanelLaunchAnimMode;

	private bool JumpPanelStopOnTimer;

	private float JumpPanelStopTimer;
	...
	private void StateGround()
	{
		...
		else
		{
			...
			IdleTimer = Time.time;
			if (!AmyStopTarotEffect && Params.AmigoName == "amy")
			{
				PlayAmyTarotFX(Enable: false);
				AmyStopTarotEffect = true;
			}
		}
	...
	private void StateVehicleSub()
	{
		...
		if (VehicleSubState == 0)
		{
			...
			_Rigidbody.MovePosition(Vector3.MoveTowards(base.transform.position + Vector3.up * 0.215f, PlayerSubPoint.position, num * 0.43f));
			if (num > 1f)
			{
	...
	private void StateSpring()
	{
		...
		if (IsGrounded() && ((SpringTimer != 0f && !SpringUseTimerToExit) ? (!flag) : (Time.time - SpringStartTime > 0.1f)))
		{
			PositionToPoint();
			StateMachine.ChangeState(StateGround);
			if (SpringLandingStop)
			{
				CurSpeed = 0f;
			}
			DoLandAnim(SpringLandingStop);
			Params.CreateLandFXAndSound();
		}
	...
	private void StateJumpPanel()
	{
		...
		bool flag2 = Time.time - JumpPanelStartTime < JumpPanelStopTimer;
		if (flag)
		{
		...
		{
			if (!JumpPanelStopOnTimer && (JumpPanelLaunchAnimMode == "jumppanel_a" || JumpPanelLaunchAnimMode == "jumppanel_d"))
			{
			...
			else if (!JumpPanelStopOnTimer && JumpPanelLaunchAnimMode == "jumppanel_b")
			{
			...
			else if ((!JumpPanelStopOnTimer && JumpPanelLaunchAnimMode == "jumppanel_c") || JumpPanelStopOnTimer)
			{
				if (((JumpPanelStopOnTimer && !flag2) || (!JumpPanelStopOnTimer && _Rigidbody.velocity.y < 0f)) && !JumpPanelAlwaysLocked)
				{
					if (!JumpPanelOnCStop)
					{
						if (CurSpeed > TopSpeed)
		...
		if (IsGrounded() && ((JumpPanelTimer != 0f && !JumpPanelUseTimerToExit) ? (!flag) : (Time.time - JumpPanelStartTime > 0.1f)))
		{
			PositionToPoint();
			StateMachine.ChangeState(StateGround);
			if (JumpPanelLandingStop)
			{
				CurSpeed = 0f;
			}
			DoLandAnim(JumpPanelLandingStop);
			Params.CreateLandFXAndSound();
		}
	...
	public override void OnSpring(Vector3 Pos, Vector3 Vel, float Timer, string LaunchAnimMode, bool UseTimerToExit, bool UseTimerToRelease, bool AlwaysLocked, bool LandingStop)
	{
		SpringPos = Pos;
		SpringVel = Vel;
		SpringTimer = Timer;
	...
	public override void OnJumpPanel(Vector3 Pos, Vector3 Vel, float Timer, string LaunchAnimMode, bool UseTimerToExit, bool UseTimerToRelease, bool AlwaysLocked, bool LandingStop, bool StopOnTimer, float StopTimer)
	{
		JumpPanelPos = Pos;
		JumpPanelVel = Vel;
		...
		JumpPanelLandingStop = LandingStop;
		JumpPanelStopOnTimer = StopOnTimer;
		JumpPanelStopTimer = StopTimer;
