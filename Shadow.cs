...
public class Shadow : PlayerBase
{
	...
	public AnimationCurve[] BlastRaiseCurves;
	...
	public override void Awake()
	{
		...
		JumpWalkSpeed = Shadow_Lua.c_jump_walk;
		JumpTopSpeed = Shadow_Lua.c_jump_run;
	...
	private void StateGround()
	{
		...
		if (!IsSlopePhys && Vector3.Dot(TargetDirection, _Rigidbody.velocity.normalized) < -0.75f && CurSpeed > WalkSpeed * 1.5f)
		{
	...
	private void StateJump()
	{
		...
		JumpAnimation = ((!(AirMotionVelocity.y > -3f)) ? 2 : ((Time.time - JumpTime > Shadow_Lua.c_jump_time_min || JumpBounce) ? 1 : 0));
		...
		if (JumpAttackSphere(base.transform.position, 0.5f, base.transform.forward * _Rigidbody.velocity.magnitude, damage))
		{
			...
			HalveSinkJump = false;
			JumpAnimation = 1;
			AirMotionVelocity.y = Shadow_Lua.c_jump_speed * 1.5f;
		}
	...
	private void StateJumpDashStart()
	{
		...
		ShadowEffects.PlayJumpDashStartFX(base.transform.forward);
	}
	...
	private void StateHomingStart()
	{
		...
		JumpdashIndex += 2;
		DontAddJumpdashIndex = false;
		ShadowEffects.PlayJumpDashStartFX(HomingDirection);
	}
	...
	public bool ChaosAttackSphere()
	{
		...
		Collider[] array = Physics.OverlapSphere(base.transform.position + base.transform.forward * 0.25f, Shadow_Lua.c_collision_homing(), base.Attack_Mask, QueryTriggerInteraction.Collide);
	...
	private void StateTornado()
	{
		...
		if (!IsSlopePhys)
		{
			FirstKickSpeed -= TopSpeed * (Time.fixedDeltaTime * ((TargetDirection == Vector3.zero) ? 0.75f : 0.2f));
		}
	...
	private void StateTrickJump()
	{
		...
		{
			AirMotionVelocity += Vector3.up * 4.25f * Time.fixedDeltaTime * ((!JumpBounce) ? 4f : 6.75f);
		}
	...
	private void StateChaosSpear()
	{
		...
		if (SpearState == 0)
		{
			...
			if (Time.time - SpearReleaseTime > Common_Lua.c_input_cd && !Singleton<RInput>.Instance.P.GetButton("Button X"))
			{
	...
	private void StateLightDash()
	{
		...
		if (splineTime < MinTime || splineTime > MaxTime)
		{
			if (IsGrounded())
			{
				CurSpeed = (TopSpeed + Shadow_Lua.c_lightdash_speed) / 1.75f;
				StateMachine.ChangeState(StateGround);
			}
			else
			{
	...
	private void StateSpinDashStart()
	{
		...
		SpindashSpd = Mathf.Min(Mathf.Max(CurSpeed, WalkSpeed * 6f), Shadow_Lua.c_spindash_spd * 1.5f);
	...
	private void StateChaosBlast()
	{
		...
		_Rigidbody.velocity = new Vector3(0f, BlastRaiseCurves[(!BlastFromGround) ? 1u : 0u].Evaluate(BlastLerpTime), 0f);
	...
	public override void FixedUpdate()
	{
		...
		if (Singleton<GameManager>.Instance.GameState != GameManager.State.Paused && StageManager.StageState != StageManager.State.Event)
		{
			if (!LockControls)
			{
				...
				{
					MaximumSpeed = (HasSpeedUp ? Shadow_Lua.c_speedup_speed_max : (IsGrounded() ? TopSpeed : JumpTopSpeed));
				}
			}
			else
			{
	...
	public override void Update()
	{
		...
		if (Singleton<GameManager>.Instance.GameState != GameManager.State.Paused && Singleton<GameManager>.Instance.GameState != GameManager.State.Result && StageManager.StageState != StageManager.State.Event && !IsDead && PlayerState != State.Talk)
		{
			...
			if (PlayerState == State.ChaosAttack)
			{
				...
				if (AttackCount < 5 && UseChaosSnap && (bool)HomingTarget && Time.time - ChaosAttackTime > 0.2f && (Singleton<RInput>.Instance.P.GetButton("Button A") || Singleton<RInput>.Instance.P.GetButtonDown("Button A")) && !ChaosAttackTarget)
				{
			...
			{
				if (Time.time - TornadoTime > Common_Lua.c_input_cd && Singleton<RInput>.Instance.P.GetButtonDown("Button A"))
				{
			...
			if (PlayerState == State.SpinDash)
			{
				...
				if (IsChaosBoost && ChaosBoostLevel == Shadow_Lua.c_level_max && ((!IsFullPower && (!HasLightMemoryShard || (HasLightMemoryShard && Singleton<RInput>.Instance.P.GetAxis("D-Pad Y") == 0f))) || IsFullPower) && !TriggerPressed && ShouldAlignOrFall(Align: false) && Singleton<RInput>.Instance.P.GetButtonDown("Right Trigger"))
				{
					StateMachine.ChangeState(StateChaosBlast);
				}
			}
	...
	private void OnFullPower(bool Trigger)
	{
		...
		Audio.PlayOneShot(LimiterSounds[(!Trigger) ? 1u : 0u], Audio.volume);
		if (Trigger)
		{
			IsFullPower = true;
			IsRestrained = false;
