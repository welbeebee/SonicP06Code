...
public class JumpPanel : ObjectBase
{
	...
	public bool LandingStop;

	public bool StopOnTimer;
	...
	private void StateJumpPanel()
	{
		...
		bool flag2 = Time.time - StartTime < StopTimer;
		if (flag)
		{
		...
		{
			if (!StopOnTimer && (LaunchAnimMode == "jumppanel_a" || LaunchAnimMode == "jumppanel_d"))
			{
			...
			else if (!StopOnTimer && LaunchAnimMode == "jumppanel_b")
			{
			...
			else if ((!StopOnTimer && LaunchAnimMode == "jumppanel_c") || StopOnTimer)
			{
				if (((StopOnTimer && !flag2) || (!StopOnTimer && PM.RBody.velocity.y < 0f)) && !PM.Base.GetPrefab("sonic_fast") && !AlwaysLocked)
				{
					if (!OnCStop)
					{
		...
		if (PM.Base.IsGrounded() && ((Timer != 0f && !UseTimerToExit) ? (!flag) : (Time.time - StartTime > 0.1f)))
		{
			...
			PM.Base.DoLandAnim(LandingStop);
	...
	private void OnTriggerEnter(Collider collider)
	{
		...
		if ((bool)component && component.GetAmigoState() != "Vehicle" && !component.IsDead)
		{
			...
			component.OnJumpPanel(base.transform.position, Direction(base.transform) * Speed, Timer, AnimMode(component.transform), UseTimerToExit, UseTimerToRelease, AlwaysLocked, LandingStop, StopOnTimer, StopTimer);
		}
	}
	...
	private void OnDrawGizmosSelected()
	{
		...
		Vector3 from = vector;
		for (float num2 = 0f; num2 < Timer; num2 += num)
		{
			vector2.y -= 9.81f * num;
			vector3 += vector2 * num;
