...
public class Jeep : VehicleBase
{
	...
	public override void Update()
	{
		...
		for (int i = 0; i < BoosterFX.Length; i++)
		{
			ParticleSystem.EmissionModule emission = BoosterFX[i].emission;
		...
		for (int j = 0; j < BoosterTrails.Length; j++)
		{
			BoosterTrails[j].colorGradient = gradient;
		}
		...
		ParticleSystem.EmissionModule emission2 = FrontWheelSmokeSandFX[0].emission;
		emission2.enabled = AxleInfos[0].LeftWheel.isGrounded && hit.collider.transform.tag == "Sand";
		emission2.rateOverTime = num;
		for (int k = 0; k < 2; k++)
		{
			ParticleSystem.EmissionModule emission3 = FrontWheelSmokeDirtFX[k].emission;
			emission3.enabled = AxleInfos[0].LeftWheel.isGrounded && hit.collider.transform.tag == "Dirt";
			emission3.rateOverTime = num;
		}
		for (int l = 0; l < 2; l++)
		{
			ParticleSystem.EmissionModule emission4 = FrontWheelSmokeSnowFX[l].emission;
			emission4.enabled = AxleInfos[0].LeftWheel.isGrounded && hit.collider.transform.tag == "Snow";
			emission4.rateOverTime = num;
		}
		for (int m = 0; m < 3; m++)
		{
			ParticleSystem.EmissionModule emission5 = FrontWheelSmokeGrassFX[m].emission;
			emission5.enabled = AxleInfos[0].LeftWheel.isGrounded && hit.collider.transform.tag == "Grass";
			emission5.rateOverTime = num;
		}
		ParticleSystem.EmissionModule emission6 = FrontWheelSmokeSandFX[1].emission;
		emission6.enabled = AxleInfos[0].RightWheel.isGrounded && hit2.collider.transform.tag == "Sand";
		emission6.rateOverTime = num;
		for (int n = 2; n < 4; n++)
		{
			ParticleSystem.EmissionModule emission7 = FrontWheelSmokeDirtFX[n].emission;
			emission7.enabled = AxleInfos[0].RightWheel.isGrounded && hit2.collider.transform.tag == "Dirt";
			emission7.rateOverTime = num;
		}
		for (int num2 = 2; num2 < 4; num2++)
		{
			ParticleSystem.EmissionModule emission8 = FrontWheelSmokeSnowFX[num2].emission;
			emission8.enabled = AxleInfos[0].RightWheel.isGrounded && hit2.collider.transform.tag == "Snow";
			emission8.rateOverTime = num;
		}
		for (int num3 = 3; num3 < 6; num3++)
		{
			ParticleSystem.EmissionModule emission9 = FrontWheelSmokeGrassFX[num3].emission;
			emission9.enabled = AxleInfos[0].RightWheel.isGrounded && hit2.collider.transform.tag == "Grass";
			emission9.rateOverTime = num;
		}
		ParticleSystem.EmissionModule emission10 = RearWheelSmokeSandFX[0].emission;
		emission10.enabled = AxleInfos[1].LeftWheel.isGrounded && hit3.collider.transform.tag == "Sand";
		emission10.rateOverTime = num;
		for (int num4 = 0; num4 < 2; num4++)
		{
			ParticleSystem.EmissionModule emission11 = RearWheelSmokeDirtFX[num4].emission;
			emission11.enabled = AxleInfos[1].LeftWheel.isGrounded && hit3.collider.transform.tag == "Dirt";
			emission11.rateOverTime = num;
		}
		for (int num5 = 0; num5 < 2; num5++)
		{
			ParticleSystem.EmissionModule emission12 = RearWheelSmokeSnowFX[num5].emission;
			emission12.enabled = AxleInfos[1].LeftWheel.isGrounded && hit3.collider.transform.tag == "Snow";
			emission12.rateOverTime = num;
		}
		for (int num6 = 0; num6 < 3; num6++)
		{
			ParticleSystem.EmissionModule emission13 = RearWheelSmokeGrassFX[num6].emission;
			emission13.enabled = AxleInfos[1].LeftWheel.isGrounded && hit3.collider.transform.tag == "Grass";
			emission13.rateOverTime = num;
		}
		ParticleSystem.EmissionModule emission14 = RearWheelSmokeSandFX[1].emission;
		emission14.enabled = AxleInfos[1].RightWheel.isGrounded && hit4.collider.transform.tag == "Sand";
		emission14.rateOverTime = num;
		for (int num7 = 2; num7 < 4; num7++)
		{
			ParticleSystem.EmissionModule emission15 = RearWheelSmokeDirtFX[num7].emission;
			emission15.enabled = AxleInfos[1].RightWheel.isGrounded && hit4.collider.transform.tag == "Dirt";
			emission15.rateOverTime = num;
		}
		for (int num8 = 2; num8 < 4; num8++)
		{
			ParticleSystem.EmissionModule emission16 = RearWheelSmokeSnowFX[num8].emission;
			emission16.enabled = AxleInfos[1].RightWheel.isGrounded && hit4.collider.transform.tag == "Snow";
			emission16.rateOverTime = num;
		}
		for (int num9 = 3; num9 < 6; num9++)
		{
			ParticleSystem.EmissionModule emission17 = RearWheelSmokeGrassFX[num9].emission;
			emission17.enabled = AxleInfos[1].RightWheel.isGrounded && hit4.collider.transform.tag == "Grass";
			emission17.rateOverTime = num;
		}
		Renderer.GetPropertyBlock(PropBlock, 3);
		PropBlock.SetFloat("_Intensity", Mathf.Lerp(PropBlock.GetFloat("_Intensity"), (IsMounted && Singleton<RInput>.Instance.P.GetButton("Button X")) ? 2f : (IsMounted ? 0.5f : 0f), Time.deltaTime * 25f));
		Renderer.SetPropertyBlock(PropBlock, 3);
		...
		if (IsMounted)
		{
			if ((bool)PM)
			{
				float num10 = Time.deltaTime * 2f;
				num10 = num10 * num10 * (3f - 2f * num10);
				float num11 = Mathf.Lerp(0f, 1f, ScaledVelSpd() / Vehicle_Param_Jeep_Lua.f_MaxSpeed);
				PM.Base.CamOffset.x = Mathf.Lerp(PM.Base.CamOffset.x, (VehicleState == 1) ? (Singleton<RInput>.Instance.P.GetAxis("Left Stick X") + 0.35f) : 0f, num10);
				PM.Base.CamOffset.y = Mathf.Lerp(PM.Base.CamOffset.y, Common_Lua.c_camera.y + ((AxleInfos[0].LeftWheel.isGrounded && AxleInfos[0].RightWheel.isGrounded && AxleInfos[1].LeftWheel.isGrounded && AxleInfos[1].RightWheel.isGrounded) ? 0.1f : (-0.25f)), Time.deltaTime * 2f);
				PM.Base.CamOffset.z = Mathf.Lerp(PM.Base.CamOffset.z, UseBooster ? 2f : num11, UseBooster ? (Time.deltaTime * 2f) : num10);
			}
	...
	private IEnumerator Reload()
	{
		...
		CurAmmo++;
		Audio.PlayOneShot(WeaponReload, Audio.volume * 0.325f);
	}

	private void OnCollisionStay(Collision collision)
	{
		if (AirLaunch && !(collision.transform.parent == base.transform) && collision.gameObject.layer != LayerMask.NameToLayer("PlayerCollision") && collision.gameObject.layer != LayerMask.NameToLayer("PlayerCollisionObject") && Time.time - LaunchTime > 1f)
		{
			AirLaunch = false;
		}
	}
