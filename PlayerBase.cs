...
public class PlayerBase : MonoBehaviour
{
	...
	internal float JumpWalkSpeed;

	internal float JumpTopSpeed;
	...
	internal float CamTargetFactor = 1f;
	...
	internal LayerMask Collision_Mask => LayerMask.GetMask("Ignore Raycast", "Water", "UI", "PlayerCollision", "PlayerCollisionObject", "BreakableObj", "Object/PlayerOnlyCol", "Vehicle");

	internal LayerMask FrontalCol_Mask => LayerMask.GetMask("Default", "DefaultObject", "PlayerCollision", "PlayerCollisionObject", "BreakableObj", "Object/PlayerOnlyCol", "AttackableSolid", "Vehicle");

	private LayerMask FastFrontalCol_Mask => LayerMask.GetMask("Default", "DefaultObject", "PlayerCollision", "PlayerCollisionObject", "Object/PlayerOnlyCol");

	internal LayerMask Attack_Mask => LayerMask.GetMask("BreakableObj", "EnemyTrigger", "Player/AttackableObj", "AttackableSolid");
	...
	private void StateGrindingStart()
	{
		...
		if (GetPrefab("silver"))
		{
			PlayerManager.silver.CanTeleDash = true;
		}
	}
	...
	private void StateGrinding()
	{
		...
		base.transform.rotation = Quaternion.Lerp(base.transform.rotation, Quaternion.LookRotation(RailData.tangent * GrindDir, Normal), Time.deltaTime * 20f);
	...
	public void StateWarpHole()
	{
		...
		_Rigidbody.velocity = Vector3.zero;
		CurSpeed = 0f;
		float num = Mathf.PerlinNoise(base.transform.position.x * 0.1f, base.transform.position.z * 0.1f) * 2f - 1f;
		float num2 = Mathf.PerlinNoise(base.transform.position.z * 0.1f, base.transform.position.y * 0.1f) * 2f - 1f;
		float num3 = Mathf.PerlinNoise(base.transform.position.z * 0.1f, base.transform.position.y * 0.1f) * 2f - 1f;
		Quaternion quaternion = Quaternion.AngleAxis(num * 25f, Vector3.left);
		Quaternion quaternion2 = Quaternion.AngleAxis(num2 * 25f, Vector3.up);
		Quaternion quaternion3 = Quaternion.AngleAxis(num3 * 25f, Vector3.forward);
		BodyTransform.rotation *= quaternion * quaternion2 * quaternion3;
		if (WarpState == 0)
		{
			float num4 = Time.time - WarpTime;
			float num5 = Mathf.Clamp01(Time.time - WarpTime) / 3f;
			num5 *= num5;
			base.transform.position = Vector3.Lerp(base.transform.position + Vector3.up * 0.05f, WarpPos, num5);
			BodyTransform.localScale = Vector3.Lerp(BodyTransform.localScale, Vector3.zero, num5);
			if ((bool)ShieldObject)
			{
				ShieldObject.transform.localScale = Vector3.Lerp(ShieldObject.transform.localScale, Vector3.zero, num5);
			}
			if (WarpType == 1 && num4 >= 2.5f)
			{
				WarpState = 1;
				WarpTime = Time.time;
				base.transform.position = WarpTargetPos;
				Camera.UncancelableEvent = false;
				Camera.StateMachine.ChangeState(Camera.StateNormal);
				Camera.transform.position = Camera.Target.position - base.transform.forward * Camera.Distance;
			}
		}
		else
		{
			float num6 = Mathf.Clamp01(Time.time - WarpTime) / 3f;
			num6 *= num6;
			BodyTransform.localScale = Vector3.Lerp(BodyTransform.localScale, Vector3.one, num6);
			if ((bool)ShieldObject)
			{
				ShieldObject.transform.localScale = Vector3.Lerp(ShieldObject.transform.localScale, Vector3.one * ((!GetPrefab("omega")) ? 1f : 1.5f), num6);
			}
			if (Time.time - WarpTime > 2.5f)
			{
				SetMachineState("StateAir");
	...
	public virtual void Update()
	{
		...
		if (Singleton<GameManager>.Instance.GameState != GameManager.State.Paused && Singleton<GameManager>.Instance.GameState != GameManager.State.Result && StageManager.StageState != StageManager.State.Event && !IsDead && GetState() != "Talk")
		{
			if (GetState() == "Grinding" && !GetPrefab("snow_board"))
			{
				if ((GetRailInput() == 0f || (GetRailInput() < 0f && !CanChangeRail(Invert: true)) || (GetRailInput() > 0f && !CanChangeRail())) && Singleton<RInput>.Instance.P.GetButtonDown("Button A") && GrindUpdateTime > Common_Lua.c_input_cd && !RailSwitch)
				{
					if (GetPrefab("silver"))
					{
						PlayerManager.silver.AirActionTime = Time.time;
					}
					if (GetPrefab("amy"))
					{
						PlayerManager.amy.OnJump();
					}
					else
					{
	...
	public void OnWarpHoleEnter(int Type, Vector3 Pos, Vector3 TargetPos)
	{
		WarpType = Type;
		WarpPos = Pos;
	...
	public void DoLandAnim(bool Force = false)
	{
		if ((!Force && CurSpeed <= 0f && _Rigidbody.velocity.y < -10f && !GetPrefab("sonic_fast") && !GetPrefab("snow_board")) || Force)
		{
	...
	internal void AccelerationSystem(float AccFloat)
	{
		...
		{
			CurSpeed -= BrakeSpeed * Time.fixedDeltaTime * (IsGrounded() ? 1.5f : 1.25f);
		}
		if (CurSpeed <= 0f)
		{
			CurSpeed = 0f;
		}
		if (CurSpeed > WalkSpeed && TurnDeltaAbs() > Common_Lua.c_turn_speed_damp)
		{
			CurSpeed -= BrakeSpeed * Time.fixedDeltaTime * (IsGrounded() ? 1.5f : 1.25f);
		}
	}

	public float TurnDeltaAbs()
	{
		return Mathf.Abs(Vector3.Angle(base.transform.forward, TargetDirection));
	}
	...
	private void UpdateEnvironmentalHazards()
	{
		...
		if (IsSinking)
		{
			if (GetState() == "BoundAttack" || GetState() == "SpinDash" || GetState() == "PsychoShock" || GetState() == "Quake" || GetState() == "KickDive" || GetState() == "SpinningClaw")
			{
				SetMachineState("StateGround");
			}
	...
	public bool AttackSphere(Vector3 Position, float Radius, Vector3 Force, int Damage, string Type = "")
	{
		...
		Collider[] array = Physics.OverlapSphere(Position, Radius, Attack_Mask, QueryTriggerInteraction.Collide);
		for (int i = 0; i < array.Length; i++)
		{
	...
	{
		if ((Camera.CameraState == PlayerCamera.State.EventFadeIn || Camera.CameraState == PlayerCamera.State.EventBFadeIn || Camera.CameraState == PlayerCamera.State.EventCFadeIn) && Camera.parameters.Mode != 1 && Camera.parameters.Mode != 10 && Camera.parameters.Mode != 11 && Camera.parameters.Mode != 2)
		{
			CamOffset.y = Mathf.Lerp(CamOffset.y, (!GetPrefab("omega")) ? 0.15f : 0.55f, Time.deltaTime * 7.5f);
		}
		else if (Camera.CameraState == PlayerCamera.State.Event && Camera.parameters.Mode != 1 && Camera.parameters.Mode != 10 && Camera.parameters.Mode != 11 && Camera.parameters.Mode != 2)
		{
		...
		else if (!GetPrefab("snow_board"))
		{
			if (GetState() != "Vehicle")
			{
				CamOffset.y = Mathf.Lerp(CamOffset.y, (!GetPrefab("omega")) ? (Common_Lua.c_camera.y - 0.2f) : (Common_Lua.c_camera.y + 0.2f), Time.deltaTime * 7.5f);
				CamOffset.z = Mathf.Lerp(CamOffset.z, 0f, Time.deltaTime * 2.5f);
			}
		}
		else
		{
			CamOffset.y = Mathf.Lerp(CamOffset.y, Common_Lua.c_camera.y - (IsGrounded() ? (-0.125f) : 0.2f), Time.deltaTime * 7.5f);
		}
		if (Camera.MultDistance && Camera.parameters.Mode == 103 && IsGrounded())
		{
			CamOffset.x = Mathf.Lerp(CamOffset.x, (Camera.parameters.Target.x == 0f) ? (-2f) : 2f, Time.deltaTime * 2.5f);
		}
		else if (!GetPrefab("snow_board") && GetState() != "Vehicle")
		{
			CamOffset.x = Mathf.Lerp(CamOffset.x, 0f, Time.deltaTime * 2.5f);
		}
		if ((Camera.CameraState == PlayerCamera.State.EventFadeIn || Camera.CameraState == PlayerCamera.State.EventBFadeIn || Camera.CameraState == PlayerCamera.State.EventCFadeIn) && (Camera.parameters.Mode == 1 || Camera.parameters.Mode == 10 || Camera.parameters.Mode == 11 || Camera.parameters.Mode == 2))
		{
			CamTargetFactor = Mathf.Lerp(CamTargetFactor, 0f, Time.deltaTime * 7.5f);
		}
		else if (Camera.CameraState == PlayerCamera.State.Event && (Camera.parameters.Mode == 1 || Camera.parameters.Mode == 10 || Camera.parameters.Mode == 11 || Camera.parameters.Mode == 2))
		{
			CamTargetFactor = 0f;
		...
		{
			CamTargetFactor = Mathf.Lerp(CamTargetFactor, 1f, Time.deltaTime * 7.5f);
		}
		...
		else
		{
			...
			Vector3 vector2 = Camera.transform.right * CamOffset.x + CameraTarget.up * CamOffset.y + base.transform.forward * Common_Lua.c_camera.z + base.transform.right * Common_Lua.c_camera.x;
			CameraTarget.position = base.transform.position + base.transform.up * 0.35f + vector2 + Camera.transform.right * CamDirDot;
		}
	}
